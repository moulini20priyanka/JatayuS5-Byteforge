...
// ── Schema check — SQL Server version ────────────────────────────────────────
async function ensureSchema() {
  try {
    // SQL Server: check column existence via sys.columns
    const [cols] = await db.query(
      `SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS
       WHERE TABLE_NAME = 'candidates' AND COLUMN_NAME = 'must_change_password'`
    );
    if (cols.length === 0) {
      await db.query(
        `ALTER TABLE candidates ADD must_change_password SMALLINT NOT NULL DEFAULT 1`
      );
...
async function nextStudentId() {
  // SQL Server: no REGEXP, use LIKE + TOP 1
  const [rows] = await db.query(
    `SELECT TOP 1 id FROM candidates WHERE id LIKE 's_%' ORDER BY LEN(id) DESC, id DESC`
  );
...
<tr><td style="background:linear-gradient(135deg,#1e3a8a,#2563eb,#3b82f6);padding:40px;text-align:center;">
  ...
  <div style="font-size:13px;color:rgba(255,255,255,.78);">Your account is ready</div>
</td></tr>
<tr><td style="padding:36px;">
  <p style="font-size:16px;font-weight:700;color:#1e293b;margin:0 0 8px;">Hi ${name},</p>
  <p style="font-size:14px;color:#64748b;line-height:1.7;margin:0 0 24px;">Your student account has been created on <strong>NeuroAssess</strong>.</p>
  ...
  <table width="100%" cellpadding="0" cellspacing="0"><tr><td align="center">
    <a href="${setPasswordUrl}" style="display:inline-block;background:linear-gradient(135deg,#1e3a8a,#2563eb);color:#fff;text-decoration:none;font-size:14px;font-weight:700;padding:14px 40px;border-radius:50px;">Log In &amp; Set Password →</a>
  </td></tr></table>
</td></tr>
</table></td></tr></table>
</body></html>`;
...
router.post('/import/parse', authenticateToken, authorizeAdmin, handleUpload, async (req, res) => {
  ...
  try {
    ...
    let wb;
    if (ext === 'csv') { const text = req.file.buffer.toString('utf-8'); wb = XLSX.read(text, { type: 'string', FS: delimiter }); }
    else { wb = XLSX.read(req.file.buffer, { type: 'buffer' }); }
    const ws   = wb.Sheets[wb.SheetNames[0]];
    const rows = XLSX.utils.sheet_to_json(ws, { defval: '' });
    if (!rows.length) return res.status(400).json({ success: false, message: 'File is empty or has no data rows.' });
    if (rows.length > 5000) return res.status(400).json({ success: false, message: 'File exceeds 5,000 rows. Split into smaller batches.' });
    const columns   = Object.keys(rows[0]);
    const sessionId = crypto.randomUUID();
    getSessions(req).set(sessionId, { rows, expiresAt: Date.now() + 3_600_000 });
    return res.json({ success:true, sessionId, columns, totalRows:rows.length, preview:rows.slice(0,5), autoMapping:buildAutoMapping(columns), fields:IMPORT_FIELDS });
  } catch (err) {
...
router.post('/import/validate', authenticateToken, authorizeAdmin, async (req, res) => {
  ...
  if (!mapping || !sessionId) return res.status(400).json({ success: false, message: 'mapping and sessionId are required.' });
  const session = getSessions(req).get(sessionId);
  if (!session) return res.status(400).json({ success: false, message: 'Import session expired. Please re-upload your file.' });

  const data = session.rows;
  const get  = (row, field) => { const col = mapping[field]; return col ? String(row[col] ?? '').trim() : ''; };

  const errors = []; let ready = 0; let skipped = 0;
  ...
  for (let i = 0; i < data.length; i++) {
    const row = data[i]; const rowNum = i + 1; const rowErrs = [];
    const name = get(row,'name'); const email = get(row,'email').toLowerCase();
    const cgpa = get(row,'cgpa'); const tenth = get(row,'tenth_percentage'); const twelfth = get(row,'twelfth_percentage');
    if (!name)  rowErrs.push({ field:'name',  reason:'Full name is required' });
    if (!email) rowErrs.push({ field:'email', reason:'Email is required' });
    else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) rowErrs.push({ field:'email', reason:'Invalid email format' });
    else if (seenEmails.has(email))  rowErrs.push({ field:'email', reason:'Duplicate email within this file' });
    else if (existingSet.has(email)) rowErrs.push({ field:'email', reason:'Email already exists in the system — will be skipped' });
    if (email) seenEmails.add(email);
    if (cgpa    && (isNaN(+cgpa)    || +cgpa    < 0 || +cgpa    > 10))  rowErrs.push({ field:'cgpa',               reason:'CGPA must be between 0 and 10' });
    if (tenth   && (isNaN(+tenth)   || +tenth   < 0 || +tenth   > 100)) rowErrs.push({ field:'tenth_percentage',   reason:'10th % must be between 0 and 100' });
    if (twelfth && (isNaN(+twelfth) || +twelfth < 0 || +twelfth > 100)) rowErrs.push({ field:'twelfth_percentage', reason:'12th % must be between 0 and 100' });
    if (rowErrs.length) { skipped++; errors.push({ row:rowNum, errors:rowErrs }); } else ready++;
  }
  return res.json({ success:true, total:data.length, ready, skipped, errors, unmappedRequired:['name','email'].filter(f=>!mapping[f]).length });
});

router.post('/import/execute', authenticateToken, authorizeAdmin, async (req, res) => {
  const { mapping, sessionId, duplicateHandling = 'skip', sendWelcomeEmails = true } = req.body;
  if (!mapping || !sessionId) return res.status(400).json({ success:false, message:'mapping and sessionId are required.' });
  const session = getSessions(req).get(sessionId);
  if (!session?.rows?.length) return res.status(400).json({ success:false, message:'Import session expired. Please re-upload your file.' });
  ...
  const adminId = req.user?.id; const adminUsername = req.user?.username || req.user?.email || 'Unknown';
  ...
  try {
    ...
    if (allEmails.length) {
      const placeholders = allEmails.map(() => '?').join(',');
      const [rows] = await db.query(`SELECT email, id FROM candidates WHERE email IN (${placeholders})`, allEmails);
      existingMap = new Map(rows.map(r => [r.email.toLowerCase(), r.id]));
    }

    const errorRows=[]; const toProcess=[]; const seenEmails=new Set();
    for (let i = 0; i < data.length; i++) {
      const row=data[i]; const name=get(row,'name'); const email=get(row,'email').toLowerCase();
      if (!name||!email||!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) { errorRows.push({row:i+1,error:!name?'Full name required':'Valid email required'}); continue; }
      if (seenEmails.has(email)) { errorRows.push({row:i+1,error:`Duplicate email in file: ${email}`}); continue; }
      seenEmails.add(email);
      const existingId = existingMap.get(email);
      if (existingId && duplicateHandling==='skip') { errorRows.push({row:i+1,error:`${email} already exists — skipped`}); continue; }
      const tempPassword=generateTempPassword(); const cgpa=get(row,'cgpa');
      toProcess.push({ name, email, tempPassword, college:get(row,'college')||'', branch:get(row,'branch')||'', batch:get(row,'batch')||'', cgpa:cgpa?parseFloat(cgpa):null, tenth_percentage:get(row,'tenth_percentage')?parseFloat(get(row,'tenth_percentage')):null, twelfth_percentage:get(row,'twelfth_percentage')?parseFloat(get(row,'twelfth_percentage')):null, backlogs:get(row,'backlogs')?parseInt(get(row,'backlogs')):null, existingId });
    }

    const HASH_BATCH=20;
    for (let i=0;i<toProcess.length;i+=HASH_BATCH) { await Promise.all(toProcess.slice(i,i+HASH_BATCH).map(async u=>{u.passwordHash=await bcrypt.hash(u.tempPassword,10);})); }

    const total=toProcess.length; let done=0; const CHUNK=100;
    const toInsert=toProcess.filter(u=>!u.existingId); const toUpdate=toProcess.filter(u=>u.existingId);
    ...
    for (let i=0;i<toInsert.length;i+=CHUNK) {
      ...
      for (const u of chunk) {
        try {
          ...
          await db.query(
            `INSERT INTO candidates (id,name,email,password_hash,college,branch,batch,cgpa,tenth_percentage,twelfth_percentage,backlogs,status,must_change_password,created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,'active',1,GETDATE())`,
            [newId,u.name,u.email,u.passwordHash,u.college,u.branch,u.batch,u.cgpa,u.tenth_percentage,u.twelfth_percentage,u.backlogs]
          );
          u.insertedId=newId; done++;
        } catch (rowErr) { errorRows.push({row:u.email,error:rowErr.message}); }
      }
    ...
    for (let i=0;i<toUpdate.length;i+=CHUNK) {
      ...
      for (const u of chunk) {
        try {
          await db.query(
            `UPDATE candidates SET name=?,college=?,branch=?,batch=?,cgpa=?,tenth_percentage=?,twelfth_percentage=?,backlogs=?,password_hash=?,must_change_password=1 WHERE id=?`,
            [u.name,u.college,u.branch,u.batch,u.cgpa,u.tenth_percentage,u.twelfth_percentage,u.backlogs,u.passwordHash,u.existingId]
          );
          u.insertedId=u.existingId; done++;
        } catch (rowErr) { errorRows.push({row:u.email,error:rowErr.message}); }
      }
      send({phase:'importing',done,total});
    }
    ...
    if (sendWelcomeEmails) {
      ...
      setImmediate(async()=>{
        for (const u of successOnes) {
          try { await sendStudentWelcomeEmail({name:u.name,email:u.email,tempPassword:u.tempPassword,setPasswordUrl:loginUrl}); }
          catch (emailErr) { console.warn(`[candidates] Email failed for ${u.email}:`,emailErr.message); }
    ...
    try { await AuditLogger.logCandidateBulkImport(adminId,adminUsername,done,`Bulk CSV/XLSX Import (${done} of ${data.length} processed)`,ipAddress,userAgent); }
    catch (auditErr) { console.error('[AuditLogger] logCandidateBulkImport failed:',auditErr.message); }

    send({ complete:true, imported:done, skipped:skippedCount, total:data.length, failed:failedCount, errors:errorRows.slice(0,50), emailNote:sendWelcomeEmails?'Welcome emails with temp passwords are being sent in the background.':'Emails skipped.' });
    res.end();
  } catch (err) {
    console.error('[candidates/import/execute]',err.message);
    send({error:true,message:`Import failed: ${err.message}`}); res.end();
  }
});
...
router.get('/export', authenticateToken, authorizeAdmin, async (req, res) => {
  ...
  try {
    // SQL Server: FORMAT() instead of DATE_FORMAT()
    let query = `SELECT name AS [Full Name], email AS [Email], college AS [College],
                        ...
                        CONVERT(VARCHAR(10), created_at, 120) AS [Created At]
...
router.get('/export/sample', authenticateToken, authorizeAdmin, async (req, res) => {
  ...
  if (format === 'csv') {
    const ws = XLSX.utils.json_to_sheet(sampleData); const csv = XLSX.utils.sheet_to_csv(ws);
    res.setHeader('Content-Disposition','attachment; filename="candidates_import_sample.csv"');
    res.setHeader('Content-Type','text/csv; charset=utf-8');
    return res.send(csv);
  }

  let ExcelJS; try { ExcelJS = require('exceljs'); } catch { ExcelJS = null; }
  if (!ExcelJS) {
  ...
  const workbook=new ExcelJS.Workbook(); const mainSheet=workbook.addWorksheet('Candidates Import'); const lookups=workbook.addWorksheet('_Lookups'); lookups.state='veryHidden';
  COLLEGES.forEach((v,i)=>{lookups.getCell(i+1,1).value=v;}); BRANCHES.forEach((v,i)=>{lookups.getCell(i+1,2).value=v;}); BATCHES.forEach((v,i)=>{lookups.getCell(i+1,3).value=v;});
  mainSheet.columns=[{header:'name',key:'name',width:22},{header:'email',key:'email',width:28},{header:'college',key:'college',width:12},{header:'branch',key:'branch',width:10},{header:'batch',key:'batch',width:10},{header:'cgpa',key:'cgpa',width:8},{header:'tenth_percentage',key:'tenth_percentage',width:12},{header:'twelfth_percentage',key:'twelfth_percentage',width:12},{header:'backlogs',key:'backlogs',width:10}];
  mainSheet.getRow(1).eachCell(cell=>{cell.font={bold:true,color:{argb:'FFFFFFFF'}};cell.fill={type:'pattern',pattern:'solid',fgColor:{argb:'FF2563EB'}};cell.alignment={vertical:'middle',horizontal:'center'};});
  sampleData.forEach(row=>mainSheet.addRow(row));
  const dvConfig=[{col:'C',values:COLLEGES,label:'college'},{col:'D',values:BRANCHES,label:'branch'},{col:'E',values:BATCHES,label:'batch'}];
  for (const {col,values,label} of dvConfig) { mainSheet.dataValidations.add(`${col}2:${col}5001`,{type:'list',allowBlank:true,formulae:[`"${values.join(',')}"`],showErrorMessage:true,errorTitle:'Invalid value',error:`Please select a valid ${label} from the list.`}); }
...
router.get('/debug', async (req, res) => {
  try {
    const [rows] = await db.query('SELECT TOP 20 id, name, college, branch, batch, status FROM candidates');
  ...
  try {
    ...
    const evalStats = await Promise.all(rows.map(async (row) => {
      try {
        ...
        return { college:row.college, evaluated:evalCount[0]?.count||0, hire_count:hireCount[0]?.count||0, avg_score:scoreData[0]?.avg_score||null, high_risk:highRisk[0]?.count||0 };
...
router.get('/by-college', authenticateToken, authorizeRecruiter, async (req, res) => {
  try {
    ...
    const [students] = await db.query(
      `SELECT id,name,email,college,branch,batch,cgpa,tenth_percentage,twelfth_percentage,backlogs,github_url,linkedin_url,leetcode_url,status,created_at FROM candidates WHERE college=? ORDER BY name ASC`, [college]
    );
    const enriched = await Promise.all(students.map(async s => {
      const student={...s};
      try { const [r]=await db.query(`SELECT TOP 1 total_score,github_score,leetcode_score,linkedin_score,test_score,decision,confidence,risk FROM candidate_reports WHERE student_id=? ORDER BY created_at DESC`,[s.id]); if(r[0]){student.github_score=r[0].github_score;student.leetcode_score=r[0].leetcode_score;student.total_score=r[0].total_score;student.overall_score=r[0].total_score;if(r[0].decision)student.__evaluation={decision:r[0].decision,confidence:r[0].confidence,risk:r[0].risk};} } catch {}
      if(!student.__evaluation){try{const[e]=await db.query(`SELECT TOP 1 decision,confidence,risk,recommendation FROM evaluations WHERE candidate_id=? ORDER BY created_at DESC`,[s.id]);if(e[0])student.__evaluation={decision:e[0].decision,confidence:e[0].confidence,risk:e[0].risk,recommendation:e[0].recommendation};}catch{}}
      try{const[a]=await db.query(`SELECT TOP 5 ea.id,ea.exam_id,ea.status,ea.score,e.title AS exam_name,e.company_name FROM exam_assignments ea JOIN exams e ON e.id=ea.exam_id WHERE ea.student_id=? ORDER BY ea.assigned_at DESC`,[s.id]);student.exams=a||[];}catch{student.exams=[];}
      return student;
    }));
    res.json(enriched);
...
router.post('/', authenticateToken, authorizeAdmin, async (req, res) => {
  ...
  try {
    ...
    if (existing.length) return res.status(409).json({ success:false, message:`A student with email "${cleanEmail}" already exists.` });
  ...
  try {
    const newId = await nextStudentId();
    await db.query(
      `INSERT INTO candidates (id,name,email,password_hash,college,branch,batch,cgpa,status,must_change_password,created_at) VALUES (?,?,?,?,?,?,?,?,'active',1,GETDATE())`,
    ...
    setImmediate(() => sendStudentWelcomeEmail({ name:name.trim(), email:cleanEmail, tempPassword, setPasswordUrl:loginUrl }).catch(e=>console.warn('[candidates] email failed:',e.message)));

    try {
      const { ipAddress, userAgent } = getClientInfo(req);
      await AuditLogger.logCandidateCreated(req.user?.id, req.user?.username||req.user?.email||'Unknown', { id:newId, name:name.trim(), email:cleanEmail, college, branch, batch }, ipAddress, userAgent);
    } catch (auditErr) { console.error('[AuditLogger] logCandidateCreated failed:',auditErr.message); }

    res.status(201).json({ success:true, message:`Student created. Welcome email sent to ${cleanEmail}.`, student:{ id:newId, name:name.trim(), email:cleanEmail, college, branch, batch } });
  } catch (err) {
    // SQL Server duplicate key error code is different
    if (err.number === 2627 || err.number === 2601) return res.status(409).json({ success:false, message:'A student with this email already exists.' });
...
router.put('/:id', authenticateToken, authorizeAdmin, async (req, res) => {
  ...
  const cleanEmail = email.trim().toLowerCase();
  try {
    const [existing] = await db.query('SELECT id FROM candidates WHERE email = ? AND id != ?', [cleanEmail, req.params.id]);
    if (existing.length) return res.status(409).json({ success:false, message:`Email "${cleanEmail}" is already used by another student.` });
  } catch (err) { return res.status(500).json({ success:false, message:err.message }); }
  try {
    await db.query(`UPDATE candidates SET name=?,email=?,college=?,branch=?,batch=?,cgpa=?,status=? WHERE id=?`, [name.trim(), cleanEmail, college, branch, batch, cgpa||null, status||'active', req.params.id]);
...
router.post('/set-password', authenticateToken, async (req, res) => {
  ...
  try {
    const [rows] = await db.query('SELECT password_hash, must_change_password FROM candidates WHERE id = ?', [studentId]);
    ...
    if (student.must_change_password && currentPassword) {
      ...
      if (!valid) return res.status(401).json({ success:false, message:'Temporary password is incorrect. Please check your welcome email.' });
    }
    const newHash = await bcrypt.hash(newPassword, 10);
    await db.query(`UPDATE candidates SET password_hash=?, must_change_password=0, status='active' WHERE id=?`, [newHash, studentId]);
...
router.get('/:id', authenticateToken, async (req, res) => {
  try {
    const [rows] = await db.query('SELECT id,name,email,college,branch,batch,cgpa,status,must_change_password,created_at FROM candidates WHERE id=?', [req.params.id]);
    if (!rows.length) return res.status(404).json({ error:'Candidate not found' });
    const c = rows[0];
    try { const [e]=await db.query('SELECT TOP 1 decision,confidence,risk,recommendation,overall_score AS score FROM evaluations WHERE candidate_id=? ORDER BY created_at DESC',[req.params.id]); c.evaluation=e[0]||null; } catch { c.evaluation=null; }
    try { const [a]=await db.query('SELECT TOP 10 ea.id,ea.exam_id,ea.status,ea.score,e.title AS exam_name,e.company_name FROM exam_assignments ea JOIN exams e ON e.id=ea.exam_id WHERE ea.student_id=? ORDER BY ea.assigned_at DESC',[req.params.id]); c.exams=a||[]; } catch { c.exams=[]; }
    res.json(c);
  } catch (err) { res.status(500).json({ error:err.message }); }
});
