...
async function callGroqLLM(evidenceSummary) {
  ...
  if (!parsed.decision || !["ALLOW","REJECT"].includes(parsed.decision)) throw new Error("Invalid decision: " + parsed.decision);
  if (typeof parsed.confidence !== "number") throw new Error("Non-numeric confidence");
...
function heuristicFallback(idResult, faceResult) {
  ...
  return { decision, confidence: Math.round(composite), reason: decision === "ALLOW" ? "Heuristic: identity signals sufficient." : "Heuristic: insufficient identity confidence.", details: `Composite: ${composite.toFixed(1)}%. Face: ${f}%, Name: ${n}%, Liveness: ${l}%, ID: ${i}%.`, flags, riskLevel: composite > 70 ? "LOW" : composite > 50 ? "MEDIUM" : "HIGH", recommendedAction: decision === "ALLOW" ? "Proceed with monitored exam." : "Retry verification.", source: "heuristic-fallback" };
}
