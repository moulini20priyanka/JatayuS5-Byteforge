...
function ruleBasedDecision(state) {
  ...
  const overall    = Math.round(
    scores.coding_skill * (hasTest ? 0.20 : 0.30) + scores.problem_solving * (hasTest ? 0.20 : 0.25) +
    scores.consistency  * (hasTest ? 0.15 : 0.20) + scores.professional_presence * (hasTest ? 0.15 : 0.25) +
  ...
  return {
    decision, confidence: { high: "High", medium: "Medium", low: "Low" }[confidence] || "Low", risk,
    overall_score: finalScore, dimension_scores: scores,
    decision_insights: state.insights?.map((i) => i.message) || [],
    recommendation: `Rule-based: score ${finalScore}/100.`,
    method: "rule_based", evaluated_at: new Date().toISOString(),
  };
}

function buildPrompt(state, preScreenResult) {
  const unified = state.unified_scores; const flags = state.cross_check_flags || []; const missing = state.missing_sources || [];
  const status  = state.source_status  || {}; const insights = state.insights || [];
  ...
  return trace(
    {
      name:    "decision-agent",
      runType: "chain",
      inputs:  { candidate_id: state.candidate_id || "unknown", unified_scores: state.unified_scores, flags: state.cross_check_flags || [] },
    ...
    async () => {
      ...
      if (pre && !pre.fast_track) {
        ...
        return { ...state, decision: pre.decision, confidence: pre.confidence, risk: pre.risk, overall_score: rb.overall_score, dimension_scores: rb.dimension_scores, decision_insights: [pre.reason, ...rb.decision_insights.slice(0,4)], recommendation: pre.reason, method: `rule_based:${pre.rule}`, evaluated_at: new Date().toISOString() };
      }
      if (!process.env.GROQ_API_KEY) return { ...state, ...ruleBasedDecision(state) };
      try {
        const llmResult = parseLLMResponse(await callLLM(buildPrompt(state, pre)));
        if (!llmResult) return { ...state, ...ruleBasedDecision(state) };
        const insights = pre?.fast_track ? [pre.reason, ...llmResult.decision_insights.slice(0,4)] : llmResult.decision_insights;
        return { ...state, decision: llmResult.decision, confidence: llmResult.confidence, risk: llmResult.risk, overall_score: llmResult.overall_score, dimension_scores: llmResult.dimension_scores, decision_insights: insights, recommendation: llmResult.recommendation, method: "llm", evaluated_at: new Date().toISOString() };
      } catch (err) {
        console.error("Decision LLM failed:", err.message);
