...
const requireRole = (...roles) => {
  ...
  return (req, res, next) => {
    if (!req.user || !normalized.includes(req.user.role)) {
      console.warn(`[RequireRole] DENIED — role="${req.user?.role}" required: ${normalized.join(', ')}`);
...
module.exports = { authenticateToken, authenticate, authorizeAdmin, authorizeRecruiter, requireRole };