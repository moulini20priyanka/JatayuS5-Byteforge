...
app.get('/api/question-bank', async (req, res) => {
  try {
    ...
    if (search) { sql += ' AND (topic LIKE ? OR question_text LIKE ? OR qb_id LIKE ?)'; const s=`%${search}%`; params.push(s,s,s); }
...
app.get('/api/question-bank/stats', async (req, res) => {
  try {
    ...
    const [[{ total }]] = await pool.query(`SELECT COUNT(*) AS total FROM question_bank WHERE is_active = 1`);
...
app.post('/api/question-bank/import', async (req, res) => {
  ...
  try {
    ...
    for (const q of questions) {
      ...
      saved.push({ id:qbId, _dbId:result.insertId, topic, type:qType==='mcq'?'MCQ':qType.charAt(0).toUpperCase()+qType.slice(1), difficulty:qDiff.charAt(0).toUpperCase()+qDiff.slice(1), source:'QuizForge AI', createdDate:new Date().toLocaleDateString('en-GB'), question:qText, question_text:qText, options:isTheory?[]:(q.options||[]), answer:isTheory?null:(q.answer||''), explanation:q.explanation||'', marks:isTheory?(q.marks||5):null, mark_type:isTheory?(q.mark_type||`${q.marks||5}m`):null, bloom_level:isTheory?(q.bloom_level||null):null, subject:isTheory?(q.subject||topic):null, key_points:isTheory?(Array.isArray(q.key_points)?q.key_points:[]):[], keywords:isTheory?(q.keywords||''):'' });
    }
    const source = examName ? `${examName}${sessionCode ? ` (session: ${sessionCode})` : ''}` : 'QuizForge AI — server.js inline import';
...
app.post('/api/question-bank', async (req, res) => {
  try {
    ...
    const qbId = genQBId(); const qType = QB_TYPE_MAP[type]||'mcq'; const qDiff = QB_DIFF_MAP[difficulty]||'medium';
    const [result] = await pool.query(`INSERT INTO question_bank (qb_id, topic, question_text, type, difficulty, source, created_by, created_at) VALUES (?, ?, ?, ?, ?, 'Manual', 1, NOW())`, [qbId, topic, topic, qType, qDiff]);
    res.status(201).json({ id:qbId, _dbId:result.insertId, topic, type:qType==='mcq'?'MCQ':qType.charAt(0).toUpperCase()+qType.slice(1), difficulty:qDiff.charAt(0).toUpperCase()+qDiff.slice(1), source:'Manual', createdDate:new Date().toLocaleDateString('en-GB') });
  } catch (err) { res.status(500).json({ error: err.message }); }
});

app.delete('/api/question-bank/:qbId', async (req, res) => {
  try { await pool.query('UPDATE question_bank SET is_active = 0 WHERE qb_id = ?', [req.params.qbId]); res.json({ success: true }); }
  catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.post('/api/exams/:id/approve', async (req, res) => {
  ...
  try {
    await pool.query(`UPDATE exams SET status='approved', approved_at=NOW(), start_date=?, end_date=?, duration_minutes=COALESCE(?,duration_minutes) WHERE id=?`, [new Date(start_date), new Date(end_date), duration_minutes?parseInt(duration_minutes):null, req.params.id]);
    const [[exam]] = await pool.query('SELECT * FROM exams WHERE id=?', [req.params.id]);
    if (!exam) return res.status(404).json({ error: 'Exam not found' });
    let students = [];
    if (exam.college && exam.college !== 'default') { const [r]=await pool.query(`SELECT id,name,email FROM candidates WHERE college=? AND status='active'`,[exam.college]); students=r; }
    if (!students.length) { const [r]=await pool.query(`SELECT id,name,email FROM candidates WHERE status='active'`); students=r; }
    let assigned = 0;
    for (const s of students) {
      ...
      await pool.query(`INSERT INTO exam_assignments (exam_id, student_id, exam_key, status, assigned_at) VALUES (?,?,?,'assigned',NOW())`, [exam.id, s.id, key]);
      assigned++;
    }
...
app.post('/api/exams/:id/reject', async (req, res) => {
  try { await pool.query(`UPDATE exams SET status='scheduled' WHERE id=?`, [req.params.id]); res.json({ success: true, status: 'scheduled' }); }
  catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.get('/api/student/exams', async (req, res) => {
  try {
    ...
    if (email) { const [r]=await pool.query('SELECT id FROM candidates WHERE email=? LIMIT 1',[email]); if(r.length) studentId=r[0].id; }
    if (!studentId) return res.status(401).json({ error: 'Student not found' });
    const [rows] = await pool.query(
      `SELECT e.id, e.title, e.exam_type, e.college, e.start_date, e.end_date, e.duration_minutes,
              e.total_marks, e.sections, e.status AS exam_status,
              ea.id AS assignment_id, ea.exam_key, ea.status AS assignment_status,
              ea.score, ea.submitted_at, COUNT(eq.id) AS question_count
       FROM exam_assignments ea
       JOIN exams e ON e.id = ea.exam_id
       LEFT JOIN exam_questions eq ON eq.exam_id = e.id
       WHERE ea.student_id = ? AND e.status IN ('approved','live','completed','scheduled')
       GROUP BY e.id, ea.id ORDER BY e.start_date DESC`, [studentId]
    );
    res.json({ exams: rows.map(r => ({ ...r, sections: typeof r.sections==='string'?JSON.parse(r.sections||'{}'):(r.sections||{}), company_name: r.college })) });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.post('/api/exams/validate-key', async (req, res) => {
  try {
    ...
    const [rows] = await pool.query(
      `SELECT ea.id AS assignment_id, ea.status AS assignment_status, e.id AS exam_id, e.title, e.duration_minutes, e.total_marks, e.start_date, e.end_date
       FROM exam_assignments ea JOIN exams e ON e.id = ea.exam_id
       WHERE ea.exam_key = ? AND ea.student_id = ?`, [exam_key.trim(), studentId]
    );
    ...
    const [questions] = await pool.query(`SELECT id, type, question_text, option_a, option_b, option_c, option_d, difficulty, marks FROM exam_questions WHERE exam_id = ? ORDER BY RAND()`, [row.exam_id]);
    await pool.query(`UPDATE exam_assignments SET status='started', started_at=NOW() WHERE id=?`, [row.assignment_id]);
...
app.post('/api/exams/:examId/submit', async (req, res) => {
  try {
    ...
    const [[asgn]] = await pool.query('SELECT id, status FROM exam_assignments WHERE exam_id=? AND student_id=?', [req.params.examId, studentId]);
    ...
    await pool.query(`UPDATE exam_assignments SET status='submitted', submitted_at=NOW(), score=?, answers=? WHERE id=?`, [score, JSON.stringify(answers||{}), asgn.id]);
...
async function createTables() {
  ...
  try {
    await conn.execute(`
      CREATE TABLE IF NOT EXISTS university_exam_assignments (
        id            INT AUTO_INCREMENT PRIMARY KEY,
        exam_id       INT NOT NULL,
        student_id    VARCHAR(100) NOT NULL,
        status        ENUM('assigned','started','submitted','completed','absent') DEFAULT 'assigned',
        exam_key      VARCHAR(50),
    ...
    await conn.execute(`
      CREATE TABLE IF NOT EXISTS student_exam_questions (
        ...
        UNIQUE KEY uq_asgn_type (assignment_id, question_type),
    ...
    await conn.execute(`
      CREATE TABLE IF NOT EXISTS viva_results (
        id INT AUTO_INCREMENT PRIMARY KEY, student_name VARCHAR(255), problem_name VARCHAR(255),
        submitted_code LONGTEXT, coding_score FLOAT, overall_score FLOAT, auth_score FLOAT,
        final_verdict VARCHAR(50), completed_at VARCHAR(50), created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
      )`);
    ...
    await conn.execute(`
      CREATE TABLE IF NOT EXISTS viva_answers (
        ...
        student_answer LONGTEXT, duration_secs INT, score FLOAT, technical_accuracy FLOAT,
        relevance FLOAT, completeness FLOAT, authenticity_score FLOAT, verdict VARCHAR(50),
        feedback TEXT, strengths TEXT, improvements TEXT, authenticity_reason TEXT,
    ...
    await conn.execute(`
      CREATE TABLE IF NOT EXISTS code_snapshots (
        id BIGINT AUTO_INCREMENT PRIMARY KEY, student_id VARCHAR(100) NOT NULL,
        exam_id INT NOT NULL, code LONGTEXT NOT NULL, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        INDEX idx_student_exam (student_id, exam_id), INDEX idx_exam_time (exam_id, created_at),
    ...
    await conn.execute(`
      CREATE TABLE IF NOT EXISTS plagiarism_reports (
        id BIGINT AUTO_INCREMENT PRIMARY KEY, student_id VARCHAR(100) NOT NULL,
        exam_id INT NOT NULL, score FLOAT DEFAULT 0, matched_with VARCHAR(100) DEFAULT NULL,
        change_count INT DEFAULT 0, checked_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    ...
    await conn.execute(`
      CREATE TABLE IF NOT EXISTS ai_detection_reports (
        id BIGINT AUTO_INCREMENT PRIMARY KEY, student_id VARCHAR(100) NOT NULL,
        exam_id INT NOT NULL, ai_score FLOAT DEFAULT 0, verdict VARCHAR(50) DEFAULT 'Human Written',
        confidence VARCHAR(20) DEFAULT 'Low', signals TEXT, ast_depth INT DEFAULT 0,
        unique_vars INT DEFAULT 0, avg_line_length FLOAT DEFAULT 0, comment_ratio FLOAT DEFAULT 0,
        sudden_paste TINYINT DEFAULT 0, perfect_structure TINYINT DEFAULT 0,
    ...
    await conn.execute(`
      CREATE TABLE IF NOT EXISTS audit_logs (
        ...
        username VARCHAR(255) NOT NULL DEFAULT 'System', action_type VARCHAR(100) NOT NULL,
        action_category VARCHAR(100) NOT NULL, entity_type VARCHAR(100) DEFAULT NULL,
        entity_id VARCHAR(100) DEFAULT NULL, entity_name VARCHAR(500) DEFAULT NULL,
        status VARCHAR(50) NOT NULL DEFAULT 'SUCCESS', details JSON DEFAULT NULL,
        ip_address VARCHAR(100) DEFAULT 'Unknown', user_agent TEXT DEFAULT NULL,
        timestamp DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
        INDEX idx_user_id (user_id), INDEX idx_action_type (action_type),
        INDEX idx_action_category (action_category), INDEX idx_timestamp (timestamp)
      ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4`);
...
async function checkPlagiarism(studentId, examId, studentCode) {
  const [others] = await pool.execute(
    `SELECT cs.student_id, cs.code FROM code_snapshots cs
     INNER JOIN (SELECT student_id, MAX(created_at) AS latest FROM code_snapshots WHERE exam_id=? AND student_id!=? GROUP BY student_id) t
     ON cs.student_id=t.student_id AND cs.created_at=t.latest`, [examId, studentId]
  );
  ...
  for (const row of others) {
    ...
    const combined = Math.max(stringSimilarity.compareTwoStrings(ns,no), stringSimilarity.compareTwoStrings(studentCode.replace(/\s+/g,' ').trim(), row.code.replace(/\s+/g,' ').trim()));
...
cron.schedule("*/2 * * * *", async () => {
  try {
    const [activePairs] = await pool.execute(`SELECT DISTINCT student_id, exam_id FROM code_snapshots WHERE created_at >= NOW() - INTERVAL 10 MINUTE`);
    for (const { student_id, exam_id } of activePairs) {
      ...
      const [[latest]] = await pool.execute(`SELECT code FROM code_snapshots WHERE student_id=? AND exam_id=? ORDER BY created_at DESC LIMIT 1`, [student_id, parsedExamId]);
      if (!latest?.code) continue;
      const [snapshots] = await pool.execute(`SELECT code, created_at FROM code_snapshots WHERE student_id=? AND exam_id=? ORDER BY created_at ASC`, [student_id, parsedExamId]);
      const { score, matchedWith } = await checkPlagiarism(student_id, parsedExamId, latest.code);
      const [[{ change_count }]] = await pool.execute(`SELECT COUNT(*) AS change_count FROM code_snapshots WHERE student_id=? AND exam_id=?`, [student_id, parsedExamId]);
      await pool.execute(`INSERT INTO plagiarism_reports (student_id,exam_id,score,matched_with,change_count,checked_at) VALUES (?,?,?,?,?,NOW()) ON DUPLICATE KEY UPDATE score=GREATEST(score,VALUES(score)),matched_with=IF(VALUES(score)>score,VALUES(matched_with),matched_with),change_count=VALUES(change_count),checked_at=NOW()`, [student_id, parsedExamId, score, matchedWith, change_count]);
      const ai = detectAIGeneratedCode(latest.code, snapshots);
      await pool.execute(`INSERT INTO ai_detection_reports (student_id,exam_id,ai_score,verdict,confidence,signals,ast_depth,unique_vars,avg_line_length,comment_ratio,sudden_paste,perfect_structure,checked_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,NOW()) ON DUPLICATE KEY UPDATE ai_score=VALUES(ai_score),verdict=VALUES(verdict),confidence=VALUES(confidence),signals=VALUES(signals),ast_depth=VALUES(ast_depth),unique_vars=VALUES(unique_vars),avg_line_length=VALUES(avg_line_length),comment_ratio=VALUES(comment_ratio),sudden_paste=VALUES(sudden_paste),perfect_structure=VALUES(perfect_structure),checked_at=NOW()`, [student_id, parsedExamId, ai.aiScore, ai.verdict, ai.confidence, JSON.stringify(ai.signals), ai.astDepth, ai.uniqueVars, ai.avgLineLength, ai.commentRatio, ai.suddenPaste, ai.perfectStructure]);
    }
