...
Return ONLY the JSON array.`,
        ...
        console.log(`[Aptitude Agent] ${difficulty}: ${questions.length} questions for "${topic}"`);
        ...
        if (is429 && retryCount < 2) {
          ...
          const waitMs = match
            ...
            : (retryCount + 1) * 20000;
          console.warn(`[Aptitude Agent] 429 "${topic}" ${difficulty} — waiting ${Math.round(waitMs/1000)}s (retry ${retryCount+1}/2)`);
          await sleep(waitMs);
          return { __result: [], __usage: null, __retry: true, __retryCount: retryCount + 1 };
        }
        console.error(`[Aptitude Agent] FAILED ${difficulty} "${topic}":`, err.message);
...
async function generateAptitudeBatchWithRetry(topic, count, difficulty, retryCount = 0) {
  ...
  if (!Array.isArray(result) && result?.__retry) {
    return generateAptitudeBatchWithRetry(topic, count, difficulty, result.__retryCount);
  }
