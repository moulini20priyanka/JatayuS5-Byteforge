...
// ─────────────────────────────────────────────────────────────────────────────
// 🤖 AI Student Onboarding & Validation Agent
//
// AI ENGINE: Groq — llama-3.3-70b-versatile (FREE tier, no credit card needed)
//   - Base URL: https://api.groq.com/openai/v1/chat/completions
//   - OpenAI-compatible REST API (no SDK needed — plain fetch)
//   - Env var required: GROQ_API_KEY  (get free at console.groq.com)
//   - Gracefully falls back to rule-based engine when key is absent
//
// Validation pipeline (in order):
//   Layer 1 — Rule-based:  email format, disposable domains, name heuristics
//   Layer 2 — DB checks:   exact duplicate email, near-duplicate email scan
//   Layer 3 — Batch scan:  within-file duplicate + alias detection
//   Layer 4 — AI (Groq):   semantic plausibility, pattern anomaly detection
//   Layer 5 — Scoring:     aggregate risk score 0-100, final PASS/WARN/BLOCK
//   Layer 6 — Audit log:   every flagged/blocked record written to audit_logs
//
// Exports:
//   validateStudents(students[], options)   → ValidationResult[]
//   validateSingleStudent(student, options) → ValidationResult
// ─────────────────────────────────────────────────────────────────────────────
...
// ── Groq client — plain fetch, OpenAI-compatible ──────────────────────────────
...
function getGroqApiKey() {
  ...
  if (!key) {
    console.warn('[ValidationAgent] GROQ_API_KEY not set — AI layer disabled, using rule-based only.');
  }
  return key || null;
}
...
/** Regex patterns for suspicious email usernames */
...
/** Levenshtein distance — for near-duplicate email detection */
...
/** Canonical form — strips dots and +tags from local, lowercases everything */
...
/** Rule-based validator — returns { issues[], warnings[], riskScore } */
function ruleBasedValidate(student) {
  ...
  // ── Email ──
  if (!email) {
    issues.push({ code: 'MISSING_EMAIL',        message: 'Email address is required',               severity: 'error' });
    risk += 40;
  } else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
    issues.push({ code: 'INVALID_EMAIL_FORMAT', message: 'Email address format is invalid',          severity: 'error' });
    risk += 40;
  } else {
    const [local, domain] = email.split('@');

    if (DISPOSABLE_DOMAINS.has(domain)) {
      issues.push({ code: 'DISPOSABLE_DOMAIN',  message: `Disposable email provider detected: @${domain}`, severity: 'error' });
      risk += 55;
    }

    for (const { re, code, msg } of SUSPICIOUS_LOCAL_PATTERNS) {
      if (re.test(local)) {
        warnings.push({ code, message: msg, severity: 'warning' });
        risk += 20;
        break;
      }
    }
    ...
    if (local.includes('+')) {
      warnings.push({ code: 'PLUS_ALIAS', message: 'Email uses + alias trick — may be masking a duplicate account', severity: 'warning' });
      risk += 15;
    }

    if (['gmail.com','yahoo.com','outlook.com','hotmail.com','rediffmail.com'].includes(domain)) {
      warnings.push({ code: 'PERSONAL_EMAIL', message: 'Personal email used instead of institutional — verify student identity', severity: 'info' });
      risk += 5;
    }
  }
  ...
  // ── Name ──
  if (!name) {
    issues.push({ code: 'MISSING_NAME',   message: 'Full name is required',                          severity: 'error' });
    risk += 30;
  } else {
    if (name.length < 3)
      warnings.push({ code: 'NAME_TOO_SHORT', message: 'Name is suspiciously short (< 3 chars)',      severity: 'warning' }), risk += 10;
    if (/^\d+$/.test(name))
      issues.push({ code: 'NUMERIC_NAME',  message: 'Name is purely numeric — certainly invalid',     severity: 'error' }),   risk += 40;
    if (/(.)\1{3,}/.test(name))
      warnings.push({ code: 'REPEATED_CHARS_NAME', message: 'Name has suspicious repeated characters', severity: 'warning' }), risk += 15;
    if (/^(test|fake|dummy|sample|admin|nobody|asdf|qwerty)/i.test(name))
      issues.push({ code: 'FAKE_NAME',     message: 'Name matches a known test/fake pattern',         severity: 'error' }),   risk += 45;
    if (name.split(' ').every(w => w.length === 1))
      warnings.push({ code: 'SINGLE_CHAR_PARTS', message: 'Name parts are single characters — verify', severity: 'warning' }), risk += 10;
  }

  // ── Batch ──
  const batchYear = parseInt(student.batch, 10);
  if (student.batch && (isNaN(batchYear) || batchYear < 2018 || batchYear > 2030)) {
    warnings.push({ code: 'UNUSUAL_BATCH', message: `Batch year ${student.batch} is outside expected range (2018–2030)`, severity: 'warning' });
    risk += 10;
  }
  ...
  if (student.cgpa !== undefined && student.cgpa !== null && student.cgpa !== '') {
    ...
    if (isNaN(cgpa) || cgpa < 0 || cgpa > 10) {
      issues.push({ code: 'INVALID_CGPA', message: `CGPA ${student.cgpa} is out of range (0–10)`, severity: 'error' });
      risk += 15;
    }
    if (cgpa === 10) {
      warnings.push({ code: 'PERFECT_CGPA', message: 'CGPA of exactly 10.0 — verify accuracy', severity: 'info' });
    }
  }
  ...
  for (const field of ['tenth_percentage', 'twelfth_percentage']) {
    if (student[field] !== undefined && student[field] !== '') {
      ...
      if (isNaN(val) || val < 0 || val > 100) {
        issues.push({ code: `INVALID_${field.toUpperCase()}`, message: `${field.replace('_', ' ')} must be 0–100`, severity: 'error' });
        risk += 10;
      }
    }
  }
...
// ─────────────────────────────────────────────────────────────────────────────
// LAYER 2 — DATABASE DUPLICATE CHECK
// ─────────────────────────────────────────────────────────────────────────────
...
async function checkDatabaseDuplicates(students) {
  ...
  try {
    ...
    const [rows] = await db.query(
      `SELECT id, name, email, college, branch, batch, created_at
       FROM candidates WHERE email IN (${placeholders})`,
      emails
    );
    const map = new Map();
...
// ─────────────────────────────────────────────────────────────────────────────
// LAYER 3 — WITHIN-BATCH NEAR-DUPLICATE DETECTION
// ─────────────────────────────────────────────────────────────────────────────

function detectNearDuplicates(students) {
  const canonicals = students.map((s, i) => ({
    i,
    raw:   (s.email || '').toLowerCase().trim(),
    canon: canonicalEmail(s.email || ''),
  }));
  ...
  for (let a = 0; a < canonicals.length; a++) {
    for (let b = a + 1; b < canonicals.length; b++) {
      ...
      if (ca.canon === cb.canon && ca.raw !== cb.raw) {
        pairs.push({ indexA: a, indexB: b, emailA: ca.raw, emailB: cb.raw, distance: 0, type: 'alias' });
        continue;
      }
      ...
      if (ca.canon && cb.canon) {
        ...
        if (dist > 0 && dist <= 2) {
          pairs.push({ indexA: a, indexB: b, emailA: ca.raw, emailB: cb.raw, distance: dist, type: 'typo' });
        }
      }
    }
  }
...
// ─────────────────────────────────────────────────────────────────────────────
// LAYER 4 — AI VALIDATION (Groq — llama-3.3-70b-versatile)
// ─────────────────────────────────────────────────────────────────────────────
...
aiVerdict rules:
- "PASS"  → aiRiskScore 0–29  (clean record)
- "WARN"  → aiRiskScore 30–59 (suspicious but allow with admin review)
- "BLOCK" → aiRiskScore 60+   (clearly fake/invalid)

aiFlags schema (only include when there are actual issues):
  ...
  try {
    ...
    await db.query(
      `INSERT INTO audit_logs
         (admin_id, admin_name, action, entity_type, entity_id, details, ip_address, user_agent, created_at)
      ...
      [
        ...
        action,
        studentEmail,
        JSON.stringify({ studentName, verdict, flags: allFlags }),
        ipAddress || 'Unknown',
        userAgent  || 'Unknown',
      ]
    );
 ...
 * validateStudents
 *
 * Main entry point — validates an array of student objects through all layers.
 *
 * @param {Array}   students  [{name, email, college, branch, batch, cgpa, ...}]
 * @param {Object}  options   {adminId, adminName, ipAddress, userAgent, writeAudit=true}
 * @returns {Promise<Array>}  ValidationResult[] in same order as input
 */
async function validateStudents(students, options = {}) {
  ...
  for (const r of results) {
    ...
    if (existing) {
      ...
      r.issues.push({
        code:     'DUPLICATE_EMAIL_DB',
        message:  `Email already in database — student ${existing.name} (ID: ${existing.id}, joined ${new Date(existing.created_at).toLocaleDateString('en-IN')})`,
        severity: 'error',
      });
  ...
  // ── Layer 3: Within-batch near-duplicates ─────────────────────────────────
  ...
  for (const pair of nearDups) {
    ...
    [pair.indexA, pair.indexB].forEach((idx, pos) => {
      ...
      results[idx].warnings.push({
        code:     'NEAR_DUPLICATE_IN_BATCH',
        message:  `Near-duplicate of row ${otherIdx + 1} — "${otherEmail}" (${typeLabel})`,
        severity: 'warning',
      });
  ...
  // ── Layer 4: AI validation in batches of 30 ───────────────────────────────
  ...
  for (let i = 0; i < students.length; i += AI_BATCH_SIZE) {
    ...
    if (aiResults) {
      for (const ar of aiResults) {
        ...
        if (!r) continue;
        r.aiVerdict = ar.aiVerdict;
        r.aiReason  = ar.aiReason;
        r.aiFlags   = ar.aiFlags || [];
  ...
  // ── Layer 6: Audit logs for flagged records ───────────────────────────────
  if (writeAudit) {
    const flagged = results.filter(r => r.verdict !== 'PASS');
    await Promise.allSettled(flagged.map(r =>
      writeValidationAudit({
        adminId, adminName,
        studentEmail: r.email,
        studentName:  r.name,
        allFlags:     [...r.issues, ...r.warnings, ...r.aiFlags],
        verdict:      r.verdict,
        ipAddress,
        userAgent,
      })
...
/** Convenience wrapper for single-student validation (Add Student form) */
