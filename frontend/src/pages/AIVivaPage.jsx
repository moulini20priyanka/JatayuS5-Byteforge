...
// AI-powered viva page that:
//  1. Pulls submitted code from localStorage (set by CodeExam after submit)
//  2. Calls POST /api/viva/generate-questions to get 3 AI questions about that code
//  3. Records spoken/typed answers with optional audio transcription
//  4. Evaluates each answer via POST /api/viva/evaluate-answer
//  5. Saves full results via POST /api/viva-results
//  6. UI mirrors CodeExam exactly: dark topbar, timer, camera panel, stats, violations
...
// ─── CSS (mirrors CodeExam palette exactly) ───────────────────────────────────
const CSS = `
...
/* ── Question panel (left, mirrors .ce-prob) ── */
.vv-qpanel{width:400px;min-width:290px;background:var(--surface);border-right:1px solid var(--border);display:flex;flex-direction:column;overflow:hidden;}
...
.vv-qbody{padding:20px 18px 80px;overflow-y:auto;flex:1;}
...
.vv-codebox{background:var(--ebg);border:1px solid var(--eline);border-radius:8px;
  ...
  white-space:pre-wrap;line-height:1.7;max-height:220px;overflow-y:auto;}
.vv-tip-box{background:var(--teal-s);border:1px solid var(--teal-b);border-radius:8px;padding:12px 14px;margin-bottom:10px;}
.vv-tip-txt{font-size:12px;color:#134e4a;line-height:1.7;}

/* ── Answer panel (right of split) ── */
...
.vv-ib-next{background:linear-gradient(135deg,#2BB1A8,#0f766e);color:#fff;}
.vv-ib-next:hover{opacity:.88;}
.vv-ib-next:disabled{background:var(--border);color:var(--dim);cursor:not-allowed;}

.vv-abody{flex:1;display:flex;flex-direction:column;padding:20px;overflow:hidden;}
.vv-ta{flex:1;background:var(--surface2);color:var(--text);font-family:'IBM Plex Sans',sans-serif;
  ...
  padding:16px;resize:none;line-height:1.7;tab-size:2;transition:border-color .15s;}
.vv-ta:focus{border-color:rgba(43,177,168,.5);background:#fff;}
.vv-ta::placeholder{color:var(--dim);}
.vv-char-hint{font-size:11px;color:var(--dim);margin-top:6px;font-family:'JetBrains Mono',monospace;}

/* Transcription status */
.vv-trans{display:flex;align-items:center;gap:8px;padding:8px 14px;background:var(--red-s);
  border:1px solid rgba(220,38,38,.2);border-radius:8px;margin-bottom:12px;}
.vv-trans-dot{width:8px;height:8px;border-radius:50%;background:var(--red);animation:vv-pulse 1s ease infinite;flex-shrink:0;}
.vv-trans-lbl{font-size:11px;font-weight:700;color:var(--red);font-family:'JetBrains Mono',monospace;}
...
// ─── Static fallback questions (used when API fails) ─────────────────────────
const FALLBACK_QUESTIONS = [
  ...
  { id: 3, type: 'EDGE CASES', question: 'What edge cases does your solution handle? Are there any inputs that could cause it to fail?', isFollowUp: true, followUpIndex: 3, totalFollowUps: 3 },
];

// ─── Webcam mock (same as CodeExam) ──────────────────────────────────────────
function WebcamMock() {
  return (
    <>
...
export default function AIVivaPage({ examId, assignmentId, onNavigate, codingScore }) {
  ...
  const [answers,     setAnswers]     = useState({}); // { [idx]: { text, eval } }
  ...
  const [timeLeft,    setTimeLeft]    = useState(20 * 60); // 20 min for viva
  ...
  // ── Load submitted code + generate questions ──────────────────────────────
  useEffect(() => {
    ...
    if (raw) {
      try {
        ...
        // last_code_submission is { [qId]: { lang, code } }
    ...
    setLoadMsg('Analyzing your code…');
  ...
  async function startRecording() {
    try {
      ...
      alert('Microphone access denied. Please use the typing mode instead.');
    }
  }
  ...
  async function transcribeAudio(blob) {
    ...
    try {
      ...
      // Silently fail — student can type their answer
    } finally {
  ...
  async function handleNext() {
    ...
    // Auto-advance after short delay to show score
    setTimeout(() => {
      setCurrentEval(null);
      setText('');
      setStartTs(Date.now());
  ...
  const handleFinalSubmit = useCallback(async (finalAnswers) => {
    ...
    const avgAuth = authScores.length
      ? Math.round(authScores.reduce((a, b) => a + b, 0) / authScores.length)
      : null;
    ...
    // Get student name from localStorage / token
    let studentName = localStorage.getItem('student_name') || localStorage.getItem('userName') || 'Unknown';
    let problemName = 'Coding Assessment';
    try {
      const raw = localStorage.getItem('last_code_submission');
      if (raw) problemName = 'Submitted Code Solution';
    } catch (_) {}
    ...
    // Also update exam_assignments if we have an assignment
    if (_assignId) {
      try {
  ...
  const pct       = timeLeft / (20 * 60);
  const tCls      = `vv-timer${pct <= 0.1 ? ' danger' : pct <= 0.25 ? ' warn' : ''}`;
  const mm        = String(Math.floor(timeLeft / 60)).padStart(2, '0');
  const ss        = String(timeLeft % 60).padStart(2, '0');
  const avgScore  = answered > 0
    ? Math.round(Object.values(answers).map(a => a.score ?? 0).reduce((a, b) => a + b, 0) / answered)
    : 0;

  const codeSnippet = submittedCode.slice(0, 600) + (submittedCode.length > 600 ? '\n…' : '');
        ...
        <div className="vv-rescard">
          ...
          <div style={{ padding: '34px 30px', textAlign: 'center' }}>
              ...
              {finalScores.map((a, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '6px 0', borderBottom: i < finalScores.length - 1 ? '1px solid var(--border)' : 'none' }}>
                  ...
                  <div style={{ display: 'flex', gap: 6 }}>
                    ...
                    {a.verdict && (
                      <span style={{ fontSize: 9, padding: '2px 7px', borderRadius: 20, background: a.verdict === 'Genuine' ? 'var(--green-s)' : 'var(--amber-s)', color: a.verdict === 'Genuine' ? 'var(--green)' : 'var(--amber)', fontWeight: 700, fontFamily: "'JetBrains Mono',monospace" }}>{a.verdict}</span>
                    )}
                  </div>
                </div>
              ))}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 20 }}>
              {[
                { icon: '📝', label: 'MCQ Round',    color: '#eff6ff', border: '#bfdbfe', text: '#1d4ed8' },
                { icon: '🗄️', label: 'SQL Round',    color: '#f5f3ff', border: '#ddd6fe', text: '#6d28d9' },
                { icon: '💻', label: 'Coding Round', color: '#ecfdf5', border: '#6ee7b7', text: '#065f46' },
                { icon: '🎤', label: 'AI Viva',      color: '#f0fdfc', border: '#99f6e4', text: '#0f766e' },
              ].map(r => (
                <div key={r.label} style={{ background: r.color, border: `1px solid ${r.border}`, borderRadius: 10, padding: '10px 12px', textAlign: 'left' }}>
                  <div style={{ fontSize: 16, marginBottom: 3 }}>{r.icon}</div>
                  <div style={{ fontSize: 11, fontWeight: 700, color: r.text }}>{r.label}</div>
                  <div style={{ fontSize: 10, color: 'var(--muted)', marginTop: 1 }}>Submitted ✓</div>
                </div>
              ))}
            </div>
  ...
  return (
    <>
      <div className="vv-wrap">
        ...
        <header className="vv-top">
          ...
          <div className="vv-einfo">
            ...
            <div className="vv-emeta">Oral Assessment · {questions.length} Questions</div>
          </div>
          {violations.length > 0 && (
          ...
          <div className={tCls}><div className="vv-tdot"/><span className="vv-tval">{mm}:{ss}</span></div>
        </header>
                ...
                <div className="vv-qtabs">
                  {questions.map((_, i) => (
                    <button
                      ...
                      onClick={() => { if (answers[i] != null) setCurrent(i); }}
                    >
                      Q{i + 1}
                    </button>
                  ))}
              ...
              <div className="vv-qbody">
                {q ? (
                  <>
                    ...
                    <div className="vv-slbl" style={{ marginTop: 18 }}>Viva Tips</div>
                    <div className="vv-tip-box">
                      <div className="vv-tip-txt">
                        {q.type?.toUpperCase().includes('LOGIC')
                          ? '💡 Walk through your code line-by-line. Explain the data structures and control flow you chose.'
                          : q.type?.toUpperCase().includes('COMPLEX')
                          ? '💡 State Big-O notation for both time and space. Justify each bound with reference to loops or recursion depth.'
                          : '💡 Think about empty inputs, large inputs, duplicates, and off-by-one errors specific to your code.'}
                      </div>
                    </div>
                    ...
                    {answers[current] && (
                      <div style={{ marginTop: 14, padding: '10px 12px', background: 'var(--green-s)', border: '1px solid rgba(22,163,74,.2)', borderRadius: 8 }}>
                        ...
                        <div style={{ fontSize: 11, color: 'var(--muted)', lineHeight: 1.5 }}>Score: {answers[current].score ?? '—'}/10</div>
                      </div>
                    )}
                  </>
                ) : (
            ...
            <div className="vv-apanel">
              <div className="vv-atop">
                ...
                <div className="vv-amode">
                  <button className={`vv-amtab${mode === 'type' ? ' active' : ''}`} onClick={() => setMode('type')}>✏️ Type</button>
                  <button className={`vv-amtab${mode === 'speak' ? ' active' : ''}`} onClick={() => setMode('speak')}>🎤 Speak</button>
                </div>
              ...
              <div className="vv-abody">
                {transcribing && (
                  <div className="vv-trans">
                    <div className="vv-trans-dot"/>
                    <span className="vv-trans-lbl">Transcribing audio…</span>
                  </div>
                )}
                ...
                <textarea
                  ...
                  placeholder={
                    mode === 'speak'
                      ? 'Click "Record" to speak your answer, or type here directly…'
                      : 'Type your answer here. Be specific — reference your code where possible…'
                  }
                  disabled={evaluating}
                />
          ...
          <div className="vv-scoresec">
            ...
            <div className="vv-scorecard">
              ...
              <div className="vv-scorelbl">
                {avgScore === 0 ? 'No answers yet' : avgScore >= 8 ? 'Excellent performance' : avgScore >= 5 ? 'Good — keep going' : 'Needs more depth'}
              </div>
            </div>

            {/* Round progress */}
            <div style={{ marginTop: 14, padding: '10px 12px', background: 'var(--surface2)', borderRadius: 8, border: '1px solid var(--border)' }}>
              <div style={{ fontSize: 9, fontWeight: 700, color: 'var(--dim)', fontFamily: "'JetBrains Mono',monospace", marginBottom: 8, letterSpacing: 1.2 }}>ROUNDS</div>
              {[
                { icon: '📝', label: 'MCQ',    done: true  },
                { icon: '🗄️', label: 'SQL',    done: true  },
                { icon: '💻', label: 'Coding', done: true  },
                { icon: '🎤', label: 'AI Viva',done: false },
              ].map(r => (
                <div key={r.label} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11, color: r.done ? 'var(--green)' : 'var(--teal)', fontWeight: r.done ? 600 : 700, marginBottom: 5 }}>
                  <span>{r.icon}</span>
                  <span style={{ flex: 1 }}>{r.label}</span>
                  <span style={{ fontSize: 9, fontFamily: "'JetBrains Mono',monospace" }}>{r.done ? '✓ Done' : '● Active'}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <div className="vv-bar">
        {recording ? (
          <button className="vv-bb" style={{ flex: 1, background: 'var(--red)', color: '#fff', borderRadius: 9, border: 'none', height: 40, fontWeight: 700, fontSize: 13, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7 }} onClick={stopRecording}>
            <IcStop size={13}/> Stop Recording
          </button>
        ) : (
          <>
            {mode === 'speak' && !recording && (
              <button
                className="vv-bb vv-bb-out"
            ...
            <button
              ...
              onClick={handleNext}
              disabled={text.trim().length < 5 || evaluating || transcribing}
            >
              {evaluating
                ? 'Evaluating…'
