...
export default function AIVivaPage({ onNavigate, codingScore, submittedCode, studentName: studentNameProp, problemName: problemNameProp }) {
  ...
  // ── TTS — speakQuestion with proper dependencies ──
  ...
  }, [aiSpeaking, screen, qIndex, activeQ?.question, speakQuestion]);
  ...
  // ── Recording with MediaRecorder + Whisper transcription ──
    ...
    return (
      <div className="vv-layout">
        <header className="vv-topbar">
          ...
          <div>
            <div className="vv-session-info">Viva Setup</div>
            <div className="vv-session-sub">Enter your details to begin</div>
          </div>
          <div className="vv-spacer" />
          <div className="vv-status-pill">
            <div className="vv-status-dot" />
            <span className="vv-status-label">READY</span>
          </div>
        </header>
        <div className="vv-body" style={{ gridTemplateColumns: "1fr", placeItems: "center" }}>
          <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 16, padding: "40px 48px", maxWidth: 520, width: "100%", boxShadow: "var(--shadow-lg)", animation: "vv-slide-up 0.4s cubic-bezier(.22,1,.36,1)" }}>
            <div style={{ fontSize: 10, fontWeight: 700, fontFamily: "'JetBrains Mono',monospace", color: "var(--accent)", letterSpacing: 2, marginBottom: 8 }}>STEP 1 OF 2</div>
            <h2 style={{ fontSize: 20, fontWeight: 700, color: "var(--text)", marginBottom: 6 }}>Start Your AI Viva</h2>
            <p style={{ fontSize: 13, color: "var(--muted)", marginBottom: 24, lineHeight: 1.6 }}>Paste your solution code below. The AI will generate personalized follow-up questions based on your implementation.</p>
            
            <div style={{ marginBottom: 16 }}>
              <label style={{ display: "block", fontSize: 11, fontWeight: 600, color: "var(--text2)", marginBottom: 6 }}>Your Name (optional)</label>
              <input type="text" value={studentName} onChange={e => setStudentName(e.target.value)} placeholder="e.g., Alex Johnson" style={{ width: "100%", padding: "10px 12px", border: `1.5px solid var(--border)`, borderRadius: 8, fontSize: 13, outline: "none", fontFamily: "inherit", transition: "border-color 0.2s" }} onFocus={e => e.target.style.borderColor = "var(--accent)"} onBlur={e => e.target.style.borderColor = "var(--border)"} />
            </div>
            
            <div style={{ marginBottom: 16 }}>
              <label style={{ display: "block", fontSize: 11, fontWeight: 600, color: "var(--text2)", marginBottom: 6 }}>Problem Name (optional)</label>
              <input type="text" value={problemName} onChange={e => setProblemName(e.target.value)} placeholder="e.g., Two Sum, Merge Intervals" style={{ width: "100%", padding: "10px 12px", border: `1.5px solid var(--border)`, borderRadius: 8, fontSize: 13, outline: "none", fontFamily: "inherit", transition: "border-color 0.2s" }} onFocus={e => e.target.style.borderColor = "var(--accent)"} onBlur={e => e.target.style.borderColor = "var(--border)"} />
            </div>
            
            <div style={{ marginBottom: 24 }}>
              <label style={{ display: "block", fontSize: 11, fontWeight: 600, color: "var(--text2)", marginBottom: 6 }}>Your Solution Code <span style={{ color: "var(--dim)", fontWeight: 400 }}>(required)</span></label>
              <textarea value={codeInput} onChange={e => setCodeInput(e.target.value)} placeholder="Paste your code here…" rows={10} style={{ width: "100%", padding: "12px 14px", border: `1.5px solid var(--border)`, borderRadius: 8, fontSize: 12, fontFamily: "'JetBrains Mono',monospace", outline: "none", resize: "vertical", transition: "border-color 0.2s", background: "var(--surface2)" }} onFocus={e => e.target.style.borderColor = "var(--accent)"} onBlur={e => e.target.style.borderColor = "var(--border)"} />
            </div>
            
            <button onClick={handleStart} disabled={!codeInput.trim()} style={{ width: "100%", padding: "13px", borderRadius: 8, border: "none", background: !codeInput.trim() ? "var(--border2)" : "var(--accent)", color: "#fff", fontSize: 13, fontWeight: 600, fontFamily: "'Inter',sans-serif", cursor: !codeInput.trim() ? "not-allowed" : "pointer", transition: "background 0.15s" }} onMouseEnter={e => { if (codeInput.trim()) e.currentTarget.style.background = "#0369a1"; }} onMouseLeave={e => { if (codeInput.trim()) e.currentTarget.style.background = "var(--accent)"; }}>
              {submittedCode ? "Regenerate Questions" : "Generate Viva Questions"} →
            </button>
            
            <div style={{ marginTop: 20, fontSize: 11, color: "var(--dim)", textAlign: "center", fontFamily: "'JetBrains Mono',monospace" }}>
              Questions are generated locally via your backend • No code is sent to external AI services
            </div>
          </div>
        </div>
      </div>
  ...
  // ── Generating / Submitting Screens ────────────────────────────────────────
  if (screen === "generating" || screen === "submitting") {
    return (
      <div className="vv-layout">
        <header className="vv-topbar">
          <div style={{ display: "flex", alignItems: "center", gap: 9 }}>
          ...
          <div className="vv-status-pill">
            <div className="vv-status-dot" />
            <span className="vv-status-label">{screen === "generating" ? "GENERATING" : "EVALUATING"}</span>
          </div>
        </header>
        <div className="vv-body" style={{ gridTemplateColumns: "1fr" }}>
          <div className="vv-gen-screen">
            <div className="vv-gen-card">
              ...
              <div className="vv-gen-title">{screen === "generating" ? "Generating Viva Questions…" : "Evaluating & Submitting…"}</div>
              <div className="vv-gen-sub">{screen === "generating" ? "AI agent is analyzing the submitted code" : "AI is scoring your answers and saving results"}</div>
    ...
    return (
      <div className="vv-layout">
        <header className="vv-topbar">
          ...
          <div className="vv-spacer" />
          {screen === "viva" && <div className="vv-q-pill">FOLLOW-UP {activeQ.followUpIndex} / {activeQ.totalFollowUps}</div>}
          <div className="vv-status-pill">
            <div className="vv-status-dot" />
    ...
    return (
      <div className="vv-layout">
        <header className="vv-topbar">
          ...
          <div className="vv-status-pill">
            <div className="vv-status-dot" />
            <span className="vv-status-label">COMPLETE</span>
          </div>
        </header>
        <div className="vv-body" style={{ gridTemplateColumns: "1fr" }}>
