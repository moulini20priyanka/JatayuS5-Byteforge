...
// Java-only coding exam (other languages removed)
...
const LANG = { key: 'java', label: 'Java', ext: 'java' };
...
.ce-exexpl{font-size:11px;color:var(--muted);margin-top:5px;line-height:1.6;}
.ce-constraint{font-size:12px;color:#334155;padding:3px 0;font-family:'JetBrains Mono',monospace;
...
/* JUDGE PANEL */
.jp-wrap{display:flex;flex-direction:column;height:100%;gap:0;}
...
.jp-bicon{font-size:16px;flex-shrink:0;padding-top:1px;}
...
.jp-sumtxt{font-size:12px;font-weight:700;}
.jp-sumtxt.all{color:var(--green);}
.jp-sumtxt.some{color:var(--red);}
...
.ce-plagbadge{padding:10px 12px;border-bottom:1px solid var(--border);}
.ce-plagbar{background:var(--border);border-radius:99px;height:5px;overflow:hidden;margin:4px 0 3px;}
.ce-plagfill{height:100%;border-radius:99px;transition:width .4s,background .4s;}
...
// ─── getTestCases — unique string IDs, no duplicates ─────────────────────────
function getTestCases(q) {
  ...
  if (meta?.testCases?.length) {
    cases = meta.testCases.map((tc, i) => ({
      ...
      explanation: tc.explanation ?? '', synthetic: false,
    }));
  }
  ...
  if (cases.length === 0) {
    for (let i = 0; i < 3; i++) {
      cases.push({ id: `tc_syn${i}`, displayId: i + 1,
        input: `Sample input ${i + 1}`, expected: `Expected output ${i + 1}`,
        hidden: false, synthetic: true,
        explanation: 'Auto-generated placeholder — add real test cases in admin.' });
    }
  }
  ...
  if (visCount > 0 && visCount < 3) {
    ...
    for (let i = visCount; i < 3; i++) {
      cases.push({ id: `tc_pad${i}`, displayId: cases.length + 1,
        input: base.input, expected: base.expected,
        hidden: false, synthetic: true, explanation: 'Verify your solution handles this case.' });
    }
  }

  if (!cases.some(c => c.hidden)) {
    cases.push({ id: 'tc_hid0', displayId: cases.length + 1,
      input: '???', expected: '???', hidden: true, synthetic: true, explanation: '' });
  }

  return cases;
}

// ─── AI / Similarity (Java-focused patterns) ───────────────────────────────────
const AI_PATTERNS = [
  { re: /HashMap<|new\s+HashMap/,          weight: 15 },
  { re: /Integer\.parseInt|Long\.parseLong/,weight: 8  },
  { re: /Arrays\.sort|Collections\.sort/,  weight: 8  },
  { re: /System\.out\.println/,            weight: 5  },
  { re: /Scanner\s+sc|Scanner\s+scanner/,  weight: 7  },
  { re: /BufferedReader|InputStreamReader/, weight: 8  },
  { re: /import\s+java\.util\.\*/,         weight: 5  },
  { re: /Map\.Entry|entrySet\(\)/,         weight: 10 },
];
const REF_SOLS = [
...
function tokenize(c) {
  return c.replace(/\/\/.*$/gm, '').replace(/\/\*[\s\S]*?\*\//g, '')
    .replace(/\s+/g, ' ').trim().split(/\b/).filter(t => t.trim().length > 0);
}
...
function detectAIAndSim(code) {
  let w = 0;
  for (const p of AI_PATTERNS) if (p.re.test(code)) w += p.weight;
  const aiScore  = Math.min(100, Math.round(w * 2));
  const maxSim   = Math.max(0, ...REF_SOLS.map(ref => jaccard(code, ref)));
  const simScore = Math.min(100, Math.round(aiScore * 0.6 + maxSim * 0.4));
  ...
  return { aiScore, simScore, verdict };
}
function pushDetection({ studentId, studentName, email, code, tcResults, totalCases, examId, examName }) {
  const { aiScore, simScore, verdict } = detectAIAndSim(code);
  const passed = tcResults.filter(r => r.passed).length;
  const record = {
    student_id: studentId || 'unknown', name: studentName || 'Student', email: email || '',
    ai_score: aiScore, similarity_score: simScore, verdict,
    viva_result: aiScore >= 50 ? 'AI Text' : 'Humanized Text',
    test_passed: passed, test_total: totalCases,
    exam_id: String(examId || 'unknown'), exam_name: examName || String(examId || 'unknown'),
    language: 'java', submitted_at: new Date().toISOString(),
    code_snippet: code.slice(0, 400), live: true,
  };
  const existing = (() => { try { return JSON.parse(localStorage.getItem('ai_detection_records') || '[]'); } catch { return []; } })();
  const idx = existing.findIndex(r => r.student_id === record.student_id && r.exam_id === record.exam_id);
  ...
  try { localStorage.setItem('detection_ping', Date.now().toString()); } catch {}
  return { aiScore, simScore, verdict };
}
...
function WebcamMock() {
  return (<>
    <span className="ce-slbl">AI Proctoring — Active</span>
          ...
          <div className="ce-chips">
            {allResults.map(tc => (
              <div key={tc.id} className={`ce-chip ${tc.passed ? 'pass' : 'fail'}`}>
                ...
                {!tc.hidden && tc.error && <span className="ce-chip-meta">{tc.error}</span>}
              </div>
            ))}
          </div>
        </>)}
...
function JudgePanel({ testCases, testResults, running, activeTabId, onTabChange }) {
  ...
  if (running) { state = 'running'; title = 'Judging on server (Java)…'; sub = 'Running each test case via /api/execute…'; icon = null; }
  else if (allRan) {
  ...
  return (
    <div className="jp-wrap">
      <div className={`jp-banner ${state}`}>
        <span className="jp-bicon">{running ? <span className="ce-spin-sm"/> : icon}</span>
        <div>
          <div className="jp-btitle">{title}</div>
          <div className="jp-bsub">{sub}
            {!running && <span style={{ marginLeft: 8, fontSize: 10, opacity: .65 }}>🖥 server (Java)</span>}
          </div>
        </div>
      </div>
      ...
      <div className="jp-casetabs">
        {visibleTCs.map(tc => (
          <button key={tc.id} className={`jp-ctab ${tabCls(tc)}`} onClick={() => onTabChange(tc.id)}>
            ...
            Case {tc.displayId}{tc.synthetic ? <span style={{ fontSize: 9, opacity: .55 }}> auto</span> : null}
          </button>
        ))}
        {hiddenTCs.map(tc => (
          <button key={tc.id} className="jp-ctab" style={{ opacity: .45, cursor: 'default' }}>
            🔒 Case {tc.displayId}
          </button>
        ))}
      </div>
      ...
      <div className="jp-body">
        {!activeTc ? (
          ...
          <div style={{ textAlign: 'center', padding: '18px', color: 'var(--dim)', fontSize: 12, fontStyle: 'italic' }}>
            🔒 Hidden — result revealed after submission
          </div>
        ) : !activeRes || activeRes.running ? (
          <div className="jp-pending">
            {activeRes?.running ? <><span className="ce-spin-sm"/> &nbsp;Running Case {activeTc.displayId} on server…</> : 'Click ▶ Run to execute Java on the backend'}
          </div>
        ) : (
          <div style={{ animation: 'ce-up .15s ease' }}>
            ...
            {activeTc.explanation && !activeRes.error && (
              <div style={{ marginTop: 8, fontSize: 11, color: 'var(--muted)', fontStyle: 'italic' }}>{activeTc.explanation}</div>
            )}
          </div>
        )}
      </div>
      ...
      {allRan && visibleTCs.length > 0 && (
        <div className={`jp-summary ${passCount === visibleTCs.length ? 'all' : 'some'}`}>
          <span className={`jp-sumtxt ${passCount === visibleTCs.length ? 'all' : 'some'}`}>
            {passCount === visibleTCs.length ? `✅ All ${visibleTCs.length} visible tests passed` : `${passCount} / ${visibleTCs.length} tests passed`}
          </span>
          <div className="jp-dots">
            {visibleTCs.map(tc => {
              const r = testResults[tc.id];
              return <div key={tc.id} className={`jp-dot ${r?.passed ? 'p' : r ? 'f' : 'n'}`} title={`Case ${tc.displayId}`}/>;
            })}
          </div>
        </div>
      )}
...
export default function CodeExam({
  ...
  useEffect(() => {
    if (!document.getElementById('ce-css5')) {
      const s = document.createElement('style'); s.id = 'ce-css5'; s.textContent = CSS;
      document.head.appendChild(s);
    }
    return () => document.getElementById('ce-css5')?.remove();
  }, []);
  ...
  const [codeMap,     setCodeMap]     = useState({});   // { [qId]: javaCode }
  ...
  const [livePlag,    setLivePlag]    = useState({ aiScore: 0, simScore: 0, verdict: 'Human Written' });
  ...
  const plagColor   = livePlag.simScore < 30 ? '#16a34a' : livePlag.simScore < 60 ? '#d97706' : '#dc2626';
  ...
  function handleTab(e) {
    ...
    handleCode(currentCode.substring(0, s) + '    ' + currentCode.substring(end)); // 4-space indent for Java
  ...
  // ── RUN — always calls /api/execute for Java ──────────────────────────────
  async function runCode() {
    ...
    try {
      const res = await fetch(`${API_URL}/api/execute`, {
        ...
        body: JSON.stringify({
          language: 'java', code: currentCode,
          test_cases: visible.map(tc => ({ input: tc.input, expected: tc.expected })),
        }),
      });

      if (!res.ok) {
        const errBody = await res.json().catch(() => ({ error: `Server ${res.status} — /api/execute not implemented` }));
      ...
      if (data.results && Array.isArray(data.results)) {
        ...
        for (let i = 0; i < visible.length; i++) {
          ...
          finalResults[tc.id] = {
            passed: Boolean(r.passed), actual: r.actual ?? r.stdout ?? null,
            error: r.error ?? r.stderr ?? null, running: false,
          };
        ...
        const plag = pushDetection({ studentId, studentName, email: localStorage.getItem('student_email') || '', code: currentCode, tcResults: resultsArr, totalCases: testCases.length, examId: String(examId || 'unknown'), examName: examTitle });
        setLivePlag(plag);
      } else {
        const singleOut = data.output ?? data.stdout ?? null;
        const singleErr = data.error  ?? data.stderr  ?? null;
        const finalResults = {};
        visible.forEach(tc => { finalResults[tc.id] = { passed: false, actual: singleOut, error: singleErr, running: false }; });
        setTestResults(finalResults);
        setOutput({ text: singleErr || singleOut || 'No output', ok: !singleErr });
      }
    } catch (err) {
      const msg = err.message || 'Unknown error';
      const finalResults = {};
      visible.forEach(tc => { finalResults[tc.id] = { passed: false, actual: null, error: msg, running: false }; });
      setTestResults(finalResults);
      setOutput({
        text: `Java execution failed: ${msg}\n\nEnsure your backend has POST /api/execute supporting Java.\nYour code is saved — submit to evaluate on server.`,
        ok: false,
      });
    }
    setRunning(false);
  ...
  const doSubmit = useCallback((auto = false) => {
    ...
    // Java is server-evaluated — mark all as "evaluated on server"
    const allTCResults = testCases.map(tc => ({ ...tc, passed: false, actual: null, error: 'Evaluated on server' }));
    const plag = pushDetection({
      studentId:   localStorage.getItem('student_id')    || 'unknown',
      studentName: localStorage.getItem('student_name')  || 'Student',
      email:       localStorage.getItem('student_email') || '',
      code: primaryCode, tcResults: allTCResults,
      totalCases: testCases.length, examId: String(examId || 'unknown'), examName: examTitle,
    });
    setLivePlag(plag);
      ...
      <div className="ce-root">
        ...
        <header className="ce-hdr">
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            {/* Java language pill — replaces language tabs */}
        ...
        <div className="ce-body">
            ...
            <div className="ce-tabs">
              {['problem', 'testcases', 'hint'].map(t => (
                <button key={t} className={`ce-tab${leftTab === t ? ' active' : ''}`} onClick={() => setLeftTab(t)}>
                  {t === 'testcases'
                    ? <>Test Cases{allRan && <span style={{ marginLeft: 5, fontSize: 10, fontWeight: 700, color: passCount === visibleTCs.length ? 'var(--green)' : 'var(--red)' }}>{passCount}/{visibleTCs.length}</span>}</>
              ...
              {!q ? (
                ...
                <>
                  ...
                  {samples.length > 0 && (<><p className="ce-slbl">Examples</p>
                    {samples.map((ex, i) => (
                      <div className="ce-ex" key={i}>
                        ...
                        {ex.explanation && <p className="ce-exexpl">{ex.explanation}</p>}
                      </div>
                    ))}</>
                  )}
                  ...
                  {!desc && !samples.length && !constraints.length && (
                    <p className="ce-desc" style={{ color: 'var(--dim)', fontStyle: 'italic' }}>Solve the problem above. Check the Test Cases tab for examples.</p>
                  )}
                </>
              ) : leftTab === 'testcases' ? (
                <JudgePanel
                  testCases={testCases} testResults={testResults} running={running}
                  activeTabId={resolvedTab} onTabChange={setActiveTabId}
                />
              ) : (
                hint
                  ? <div className="ce-hint"><p className="ce-hinttxt">{hint}</p></div>
                  : <div className="ce-empty">No hint available. Break the problem into smaller steps.</div>
              )}
            </div>
          </div>
          ...
          <div className="ce-ed">
            <div className="ce-edtop">
              {/* Static Java badge instead of language tabs */}
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <aside className="ce-side">
            ...
            <div className="ce-wlimit">
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span className="ce-slbl" style={{ margin: 0 }}>Warnings</span>
            ...
            <div className="ce-qnav">
              <span className="ce-slbl">Problems</span>
            ...
            <div style={{ padding: '12px', borderBottom: '1px solid var(--border)' }}>
              <span className="ce-slbl">Progress</span>
            ...
            <div className="ce-plagbadge">
              <span className="ce-slbl">Originality Monitor</span>
              <div style={{ background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 8, padding: '10px 12px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
                  <span style={{ fontSize: 12, color: '#334155', fontWeight: 600 }}>Code analysis</span>
                  <span style={{ fontSize: 11, fontWeight: 700, fontFamily: "'JetBrains Mono',monospace", color: plagColor }}>
                    {livePlag.simScore === 0 ? 'Pending' : livePlag.verdict}
                  </span>
                </div>
                <div className="ce-plagbar"><div className="ce-plagfill" style={{ width: `${livePlag.simScore}%`, background: plagColor }}/></div>
                <div style={{ fontSize: 9, color: 'var(--dim)', fontFamily: "'JetBrains Mono',monospace" }}>Updated on each Run</div>
              </div>
            </div>
          </aside>
        </div>
