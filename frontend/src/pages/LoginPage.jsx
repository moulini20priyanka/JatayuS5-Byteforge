...
/* ── Role-specific left panel visuals (unchanged) ── */
const AdminPanel = () => (
    ...
    {[0,1,2,3].map(i => (
      <g key={i}>
        ...
        <circle cx="54" cy={126+i*22} r="4"
          ...
          strokeWidth="1"/>
        <rect x="64" y={123+i*22} width={[80,100,70,90][i]} height="3" rx="1.5" fill={i===1 ? "rgba(255,255,255,0.7)" : "rgba(255,255,255,0.35)"}/>
      </g>
    ))}
...
export default function Login() {
  ...
  const handleLogin = async () => {
    ...
    clearSession();
    try {
      const endpoint = role === "student" ? `${API}/api/auth/student/login` : `${API}/api/auth/login`;
      ...
      if (!res.ok) { setErr(data.error || "Login failed. Please try again."); setLoading(false); return; }
      const token = data.token || data.accessToken;
      if (!token) { setErr("Login failed: no token received from server."); setLoading(false); return; }
      ...
      storeSession({ token, role: resolvedRole, name: resolvedName, email: resolvedEmail, studentId: String(resolvedId) });
      if (resolvedRole === "student") navigate("/student-dashboard");
      else if (resolvedRole === "admin") navigate("/admin-dashboard");
      else if (resolvedRole === "recruiter") navigate("/recruiter-dashboard");
  ...
  const handleKeyDown = (e) => { if (e.key === "Enter" && valid && !loading) handleLogin(); };

  const roleConfig = {
    admin: { label: "Admin", tagline: "Platform Administration", icon: <ShieldIcon />, features: ["User & Role Management", "System-Wide Analytics", "Live Exam Control"], panel: <AdminPanel />, primaryColor: "#3b82f6", lightColor: "rgba(100,200,255,0.15)" },
    recruiter: { label: "Recruiter", tagline: "Recruitment & Hiring", icon: <BriefcaseIcon />, features: ["AI Candidate Scoring", "Pipeline Tracking", "Smart Shortlisting"], panel: <RecruiterPanel />, primaryColor: "#3b82f6", lightColor: "rgba(100,200,255,0.15)" },
    student: { label: "Student", tagline: "Candidate Assessment", icon: <GradCapIcon />, features: ["Take Secure Exams", "View Your Results", "Track Progress"], panel: <StudentPanel />, primaryColor: "#3b82f6", lightColor: "rgba(100,200,255,0.15)" },
  };
  const rc = roleConfig[role] || roleConfig.student;
  const placeholders = { admin: "admin@neuroassess.com", recruiter: "recruiter@company.com", student: "student@college.edu" };
    ...
    <>
      ...
      <div style={{ 
        ...
        <div style={{ 
          ...
          top: "5%", 
          left: "3%", 
          animation: mounted ? "ringFloat1 25s ease-in-out infinite" : "none",
          opacity: 0.6 
        }} />
        <div style={{ 
          position: "absolute", 
          ...
          top: "7%", 
          left: "5%", 
          animation: mounted ? "ringFloat1 25s ease-in-out infinite reverse" : "none",
          opacity: 0.5 
        }} />
        <div style={{ 
          position: "absolute", 
        ...
        <div style={{ 
          ...
          top: "40%", 
          left: "2%", 
          animation: mounted ? "ringFloat3 28s ease-in-out infinite" : "none",
          opacity: 0.45 
        }} />
        ...
        <div style={{ 
          ...
          width: "150px", 
          height: "150px", 
          border: "2.5px solid rgba(191,219,254,0.35)", 
          borderRadius: "50%", 
          top: "43%", 
          left: "4%", 
          animation: mounted ? "ringFloat3 28s ease-in-out infinite reverse" : "none",
          opacity: 0.4 
        }} />
        ...
        <div style={{ 
          ...
          bottom: "15%", 
          left: "5%", 
          animation: mounted ? "ringFloat2 22s ease-in-out infinite" : "none",
          opacity: 0.4 
        }} />

        {/* RIGHT SIDE RINGS */}
        
        {/* Top Right - Large Ring Set */}
        <div style={{ 
          position: "absolute", 
          width: "260px", 
          height: "260px", 
          ...
          top: "8%", 
          right: "4%", 
          animation: mounted ? "ringFloat2 26s ease-in-out infinite" : "none",
          opacity: 0.55 
        }} />
        <div style={{ 
          position: "absolute", 
          ...
          top: "10%", 
          right: "6%", 
          animation: mounted ? "ringFloat2 26s ease-in-out infinite reverse" : "none",
          opacity: 0.5 
        }} />
        <div style={{ 
          position: "absolute", 
        ...
        <div style={{ 
          ...
          width: "200px", 
          height: "200px", 
          border: "2.5px solid rgba(100,200,255,0.3)", 
          borderRadius: "50%", 
          top: "42%", 
          right: "3%", 
          animation: mounted ? "ringFloat4 24s ease-in-out infinite" : "none",
          opacity: 0.45 
        }} />
        ...
        <div style={{ 
          ...
          top: "45%", 
          right: "5%", 
          animation: mounted ? "ringFloat4 24s ease-in-out infinite reverse" : "none",
          opacity: 0.4 
        }} />
        ...
        <div style={{ 
          ...
          bottom: "18%", 
          right: "6%", 
          animation: mounted ? "ringFloat2 20s ease-in-out infinite reverse" : "none",
          opacity: 0.45 
        }} />

        {/* Additional subtle rings for depth */}
        <div style={{ 
          position: "absolute", 
          width: "320px", 
          height: "320px", 
          ...
          animation: mounted ? "ringPulse 15s ease-in-out infinite" : "none",
          opacity: 0.3 
        }} />
        ...
        <div style={{
          ...
          {/* RIGHT PANEL - REFINED LAYOUT */}
          <div style={{
            flex: "1", background: "white", display: "flex", flexDirection: "column",
            padding: "48px 48px", position: "relative"
          }}>
            {/* Form container - vertically centered with balanced spacing */}
            <div style={{ 
              width: "100%", maxWidth: 380, margin: "auto", display: "flex", flexDirection: "column", gap: "0" 
            }}>
              ...
              <div style={{ textAlign: "center", marginBottom: "32px" }}>
                ...
                <p style={{ fontSize: "14px", color: "#64748b", margin: 0, lineHeight: 1.5 }}>Sign in to continue to your dashboard</p>
              </div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "20px", marginBottom: "24px" }}>
                ...
                <div>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: "12px", background: "#f8fafc", border: `2px solid ${focused === "email" ? "#3b82f6" : "#e2e8f0"}`, borderRadius: "12px", padding: "0 16px", transition: "all 0.2s ease", boxShadow: focused === "email" ? "0 0 0 4px rgba(59,130,246,0.12)" : "none" }}>
                    ...
                    <input type="email" placeholder={placeholders[role]} value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} onFocus={() => setFocused("email")} onBlur={() => setFocused("")} onKeyDown={handleKeyDown} style={{ flex: 1, padding: "14px 0", border: "none", background: "transparent", color: "#0f172a", outline: "none", fontSize: "14px", fontFamily: "'Inter', sans-serif" }} />
                  </div>
                </div>
                ...
                <div>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "8px" }}>
                    ...
                    <span style={{ fontSize: "13px", color: "#3b82f6", cursor: "pointer", fontWeight: "500" }} onClick={() => {}}>Forgot password?</span>
                  </div>
                  <div style={{ display: "flex", alignItems: "center", gap: "12px", background: "#f8fafc", border: `2px solid ${focused === "password" ? "#3b82f6" : "#e2e8f0"}`, borderRadius: "12px", padding: "0 16px", position: "relative", transition: "all 0.2s ease", boxShadow: focused === "password" ? "0 0 0 4px rgba(59,130,246,0.12)" : "none" }}>
                    ...
                    <input type={showPass ? "text" : "password"} placeholder="••••••••" value={form.password} onChange={e => setForm({ ...form, password: e.target.value })} onFocus={() => setFocused("password")} onBlur={() => setFocused("")} onKeyDown={handleKeyDown} style={{ flex: 1, padding: "14px 0", border: "none", background: "transparent", color: "#0f172a", outline: "none", fontSize: "14px", fontFamily: "'Inter', sans-serif", paddingRight: "40px" }} />
                    <button onClick={() => setShowPass(v => !v)} tabIndex={-1} style={{ background: "none", border: "none", cursor: "pointer", display: "flex", alignItems: "center", padding: "4px", position: "absolute", right: "12px", color: showPass ? "#3b82f6" : "#94a3b8", transition: "color 0.2s" }}><EyeIcon open={showPass} /></button>
                  </div>
                </div>
              </div>
              ...
              {/* Error */}
              {err && (
                <div style={{ display: "flex", alignItems: "center", gap: "8px", fontSize: "13px", color: "#dc2626", background: "#fef2f2", border: "1px solid #fecaca", borderRadius: "10px", padding: "10px 14px", marginBottom: "20px", fontWeight: 500, animation: "shake 0.4s ease" }}>
              ...
              <button disabled={!valid || loading} onClick={handleLogin} style={{
                width: "100%", padding: "14px", borderRadius: "12px", border: "none", cursor: (!valid || loading) ? "not-allowed" : "pointer",
                background: (!valid || loading) ? "#f1f5f9" : "linear-gradient(135deg, #3b82f6 0%, #2563eb 100%)",
                color: (!valid || loading) ? "#94a3b8" : "white", fontWeight: "600", fontSize: "14px", fontFamily: "'Inter', sans-serif",
                transition: "all 0.25s ease", boxShadow: (!valid || loading) ? "none" : "0 4px 14px rgba(59,130,246,0.3)",
              ...
              >
                {loading ? (
                  ...
                  <><span>Sign in to dashboard</span><ArrowRight /></>
                )}
              </button>
              ...
              <div style={{ marginTop: "auto", paddingTop: "8px" }}>
                ...
                <button onClick={() => navigate("/")} style={{
                  display: "flex", alignItems: "center", justifyContent: "center", gap: "6px", margin: "0 auto", padding: "8px 16px",
                  background: "none", border: "none", color: "#64748b", fontSize: "13px", cursor: "pointer", borderRadius: "8px",
                  fontFamily: "'Inter', sans-serif", transition: "color 0.2s", fontWeight: "500"
                }} onMouseEnter={(e) => e.target.style.color = "#475569"} onMouseLeave={(e) => e.target.style.color = "#64748b"}>
