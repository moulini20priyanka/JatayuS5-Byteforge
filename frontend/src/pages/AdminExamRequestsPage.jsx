// AdminExamRequestsPage.jsx — Admin Platform · Unified Design System v2
// ── FIXES IN THIS VERSION ────────────────────────────────────────────────────
//
//  ROOT CAUSE (primary): MySQL/mysql2 can return JSON columns (section_config,
//  eligibility_criteria, sectional_cutoffs) as Buffer objects on older configs.
//  safeParse was treating Buffer as a plain object (typeof === 'object',
//  !Array.isArray) and returning it unchanged — causing Object.entries() to
//  produce garbage key/value pairs, which threw inside normalizeRow, which was
//  silently caught and the row was dropped from the rendered list.
//
//  ROOT CAUSE (secondary): The backend GET /all query selects er.* then
//  u.company_name AS company_name — the alias overwrites er.company_name but
//  more importantly, on some MySQL strict-mode / mysql2 versions the duplicate
//  column name in the result set causes the JSON columns to be returned in an
//  unexpected position/type, breaking safeParse for every row.
//
//  FIX LIST
//  ────────
//  F-1  safeParse: Buffer guard — if value has a Buffer-like structure
//       (Uint8Array / has .buffer property), convert to string first.
//  F-2  safeParse: explicit string coercion for any non-plain-object so that
//       numbers, booleans, and Buffer-derived values all go through JSON.parse.
//  F-3  normalizeRow: wraps each JSON field in an extra try/catch with a
//       descriptive console.warn so silent failures are surfaced in dev.
//  F-4  fetchRequests: detailed error logging — logs HTTP status + body so
//       auth failures (401/403) and server errors (500) are distinguishable.
//  F-5  fetchRequests: on non-ok response, attempts to read the error body and
//       shows a specific toast (e.g. "Session expired — please log in again")
//       instead of the generic "Failed to load" message.
//  F-6  counts / filtered: added an explicit String cast before comparison as a
//       last-resort guard, in case a row slips through without normalization.
//  F-7  Table row onClick: sets selected from the normalized `requests` array
//       (looked up by id) rather than from the raw loop variable `r`, ensuring
//       the detail panel always shows fully-normalized data.
//  F-8  Status comparison in action-button rendering (table + detail panel):
//       always calls normalizeStatus() at the comparison site — belt-and-
//       suspenders on top of normalizeRow so stale state can never show wrong
//       buttons.
//  F-9  handleApprove / handleReject: after fetchRequests() re-runs, the
//       setSelected sync inside fetchRequests already handles panel update —
//       removed the redundant second setState that was causing a flicker.
//  F-10 adminId guard: parseInt result is validated with Number.isFinite;
//       falls back to null (not 1) so the backend doesn't attribute an action
//       to the wrong admin when localStorage is missing.
//  F-11 Empty-state copy: "No pending requests" shown only for the pending
//       filter; generic copy for other empty states.
//
// ─────────────────────────────────────────────────────────────────────────────
