 ...
 * TheoryExamPage.jsx
 *
 * Written / descriptive-answer exam page for university exams.
 * UI is a PIXEL-PERFECT match to ExamPage.jsx:
 *   • Same CSS variables, layout grid, topbar, sidebar
 *   • Same webcam mock, stat cards, question nav grid, legend
 *   • Same action bar, violation banner, proctoring pill, timer
 *   • Same watermark, result overlay style
 *
 * Differences from MCQ ExamPage:
 *   • Main area shows a textarea instead of radio options
 *   • Word-count meter below textarea
 *   • Skip button in action bar
 *   • "Save & Next" / "Submit Exam" in action bar
 *   • Auto-save every 30 s
 *   • Stat cards: Answered / Skipped / Remaining / Violations
 *   • Nav dots support: current / answered / skipped / pending
 */

import { useState, useEffect, useRef, useCallback } from "react";
import { useLocation } from "react-router-dom";

/* ── Fonts (same as ExamPage) ── */
...
// ── CSS — identical tokens + layout to ExamPage, extra theory classes appended ──
const CSS = `
...
/* ── Question card — IDENTICAL wrapper to ExamPage ── */
...
/* ── Theory-specific: meta badges under question ── */
...
.na-qbadge-warn   { background:var(--amber-s); color:var(--amber); border:1px solid rgba(217,119,6,0.2); }
...
/* ── Answered notice (same as ExamPage) ── */
...
/* ── Action bar — IDENTICAL position to ExamPage ── */
...
.na-btn-green { flex:1; padding:12px; background:var(--green); color:#fff; font-size:14px; box-shadow:0 2px 10px rgba(22,163,74,0.28); }
.na-btn-green:hover { background:#15803d; }

/* ── Sidebar — IDENTICAL to ExamPage ── */
...
/* ── Stat cards — IDENTICAL to ExamPage ── */
...
/* ── Nav grid — IDENTICAL to ExamPage + skipped state ── */
...
.na-sidebar-submit button { width:100%; padding:11px; background:var(--green-s); border:1px solid rgba(22,163,74,0.25); border-radius:9px; font-size:12px; font-weight:700; color:var(--green); cursor:pointer; font-family:'Inter',sans-serif; transition:all 0.15s; }
...
/* ── Result overlay — IDENTICAL structure to ExamPage ── */
...
/* ── Keyframes — IDENTICAL to ExamPage ── */
...
function resolveExamId(p) {
  ...
  return localStorage.getItem("univ_exam_id") || localStorage.getItem("exam_id") || null;
}
function resolveAssignmentId(p) {
  ...
  return localStorage.getItem("univ_assignment_id") || localStorage.getItem("assignment_id") || null;
}
function resolveExamKey() {
  ...
  return localStorage.getItem("univ_exam_key") || "";
}
...
// ── Webcam mock — IDENTICAL to ExamPage ──────────────────────────────────────
function WebcamMock() {
  return (
    <>
        ...
        <div style={{ padding:"32px 28px" }}>
          <div style={{ textAlign:"center", marginBottom:20 }}>
            <div style={{
              ...
              background:"var(--green-s)", border:"2px solid rgba(22,163,74,0.2)", color:"var(--green)",
            }}>
              <IconPencil />
            </div>
          ...
          {stats.unanswered > 0 && (
            <div style={{
              ...
              <IconWarn />
              {stats.unanswered} question{stats.unanswered > 1 ? "s" : ""} left unanswered.
            </div>
          )}
          ...
          <div style={{ display:"flex", gap:10 }}>
            ...
            <button
              ...
              style={{
                ...
                background: submitting ? "#d1d5db" : "var(--green)",
                fontSize:13, fontWeight:700, color:"#fff",
                cursor: submitting ? "not-allowed" : "pointer",
                fontFamily:"Inter,sans-serif",
                display:"flex", alignItems:"center", justifyContent:"center", gap:8,
                boxShadow: submitting ? "none" : "0 2px 10px rgba(22,163,74,0.28)",
              }}
            >
              {submitting
                ? <><span style={{ width:14, height:14, border:"2px solid rgba(255,255,255,0.3)", borderTop:"2px solid #fff", borderRadius:"50%", display:"inline-block", animation:"na-spin 0.75s linear infinite" }} /> Submitting…</>
                : "Submit Exam ✓"
              }
            </button>
          </div>
...
// ── Result overlay — styled like ExamPage result ──────────────────────────────
function ResultOverlay({ result, nextSection, onNavigate }) {
  if (!result) return null;

  const cfg = nextSection
    ? {
        badge:       "THEORY COMPLETE",
        badgeColor:  "var(--accent)",
        title:       "Theory Round Submitted",
        subtitle:    `Proceed to the ${nextSection === "coding" ? "Coding" : "SQL"} section now.`,
        boxClass:    "blue",
        btnClass:    "blue",
        iconColor:   "var(--accent)",
        icon:        <IconPencil />,
        unlockTitle: `${nextSection === "coding" ? "Coding" : "SQL"} Round Unlocked`,
        unlockSub:   `Proceed to the ${nextSection === "coding" ? "Coding Challenge" : "SQL & Database"} section.`,
        btnLabel:    `Proceed to ${nextSection === "coding" ? "Coding" : "SQL"} Round →`,
        onClick:     () => onNavigate(nextSection),
      }
    : {
        badge:       "EXAM COMPLETE",
        badgeColor:  "var(--green)",
        title:       "Theory Exam Submitted",
        subtitle:    "Your written answers have been saved. AI evaluation will score your responses.",
        boxClass:    "green",
        btnClass:    "green",
        iconColor:   "var(--green)",
        icon:        <IconTrophy />,
        unlockTitle: "All Done!",
        unlockSub:   "Your responses have been saved. You may return to the dashboard.",
  ...
  return (
    <div className="na-result-overlay show">
      <div style={{
        ...
        borderRadius:18, overflow:"hidden", maxWidth:480, width:"100%",
        ...
        <div style={{ padding:"40px 36px", textAlign:"center" }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:10, marginBottom:20 }}>
            {[
              { label:"ANSWERED", val:result.answered,   color:"var(--green)"  },
              { label:"SKIPPED",  val:result.skipped,    color:"var(--amber)"  },
              { label:"TOTAL",    val:result.total,      color:"var(--accent)" },
            ].map(s => (
              <div key={s.label} className="na-stat-card">
                <div className="na-stat-val" style={{ color:s.color }}>{s.val}</div>
                <div className="na-stat-lbl">{s.label}</div>
              </div>
            ))}
          </div>
...
export default function TheoryExamPage({
  ...
  // ── Fetch questions — primary + validate-key fallback ─────────────────────
  useEffect(() => {
    if (!examId) { setFetchError("No exam ID found. Please restart the exam flow."); setLoading(false); return; }
    let cancelled = false;
      ...
      try {
        ...
        try {
          const res = await fetch(
            `${API_URL}/api/exams/${examId}/theory-questions?assignment_id=${assignmentId || ""}`,
            { headers: { Authorization: `Bearer ${getToken()}` } }
          );
          if (res.ok) {
            const data = await res.json();
            const raw  = data.questions || data || [];
            if (Array.isArray(raw) && raw.length > 0) { qs = raw.map(normalizeQ); dur = data.duration || null; }
          }
        } catch {}

        // Attempt 2: validate-key fallback
        if (!qs) {
          const examKey = resolveExamKey();
          if (!examKey) throw new Error("No theory questions found and no exam key available.");
          const kr = await fetch(`${API_URL}/api/exams/university/validate-key`, {
            method:"POST",
            headers:{ "Content-Type":"application/json", Authorization:`Bearer ${getToken()}` },
            body: JSON.stringify({ exam_key: examKey }),
          });
          if (!kr.ok) throw new Error(`Fallback failed: ${kr.status}`);
          const data = await kr.json();
          if (!data.valid) throw new Error(data.error || "Exam key no longer valid.");
          const raw =
          ...
          if (Array.isArray(raw) && raw.length > 0) {
            qs = raw.map(normalizeQ); dur = data.duration || null;
          } else {
            const allQs = (data.questions || []).filter(q =>
              ["written","theory"].includes(q.type) || ["written","theory"].includes(q.question_type) ||
              ["written","theory"].includes(q.section)
            );
            if (allQs.length > 0) qs = allQs.map(normalizeQ);
            else throw new Error("No theory/written questions found for this exam.");
          }
        }
  ...
  // Card key + focus on question change
  useEffect(() => {
    setCardKey(k => k + 1);
  ...
  const autoSave = useCallback(async () => {
    ...
    setSaving(true);
    const endpoints = [
      `${API_URL}/api/exams/${examId}/theory-autosave`,
      `${API_URL}/api/questions/autosave`,
    ];
  ...
  const doSubmit = useCallback(async (auto = false) => {
    ...
    const answersArray = Object.entries(ans).map(([question_id, answer_text]) => ({ question_id, answer_text, selected_ans: answer_text }));
    const violLog = violationsRef.current;
    const base = { assignment_id:assignmentId, exam_id:examId, answers:ans, round:"theory", violation_count:violLog.length, violations:violLog };

    const attempts = [
      { url:`${API_URL}/api/exams/${examId}/theory-submit`,      body:{ ...base, auto_submitted:auto } },
      { url:`${API_URL}/api/exams/${examId}/submit`,             body:{ ...base, auto_submitted:auto } },
      { url:`${API_URL}/api/questions/submit`,                   body:{ assignment_id:assignmentId, exam_id:examId, answers:answersArray, round:"theory", violation_count:violLog.length, violations:violLog } },
      { url:`${API_URL}/api/assignments/${assignmentId}/submit`, body:{ ...base, answers:answersArray } },
    ];

    for (const { url, body } of attempts) {
      if (url.includes("undefined") || url.includes("null")) continue;
      try {
        const res = await fetch(url, { method:"POST", headers:{ "Content-Type":"application/json", Authorization:`Bearer ${getToken()}` }, body:JSON.stringify(body) });
        if (res.ok) break;
    ...
    <>
      ...
      <div className="na-layout">
        ...
        <header className="na-topbar">
          ...
          <div className="na-exam-info">
            ...
            <div className="na-exam-meta">{`Theory · ${questions.length} Questions`}</div>
          </div>
          ...
          {/* Auto-save indicator */}
          {saving ? (
            <div className="na-save-pill saving">
              <span style={{ width:8, height:8, border:"1.5px solid #cbd5e1", borderTopColor:"#2563eb", borderRadius:"50%", display:"inline-block", animation:"na-spin 0.75s linear infinite" }} />
              Saving…
            </div>
          ) : lastSaved ? (
            <div className="na-save-pill">
              <IconCheckGreen />
        ...
        <main className="na-main">
          {/* Progress bar — IDENTICAL to ExamPage */}
          ...
          {q && (
            <div className="na-qcard" key={cardKey}>
              {/* Question number row — IDENTICAL to ExamPage */}
              ...
              {/* Question text — IDENTICAL styling to ExamPage */}
              ...
              {/* Hint */}
              {q.hint && (
                <div className="na-hint">💡 <strong>Hint:</strong> {q.hint}</div>
              )}
              ...
              <div className="na-meter-wrap">
                <div className="na-meter-bar">
                  <div className="na-meter-fill" style={{ width:`${meterPct}%`, background:meterColor }} />
                </div>
                <div className="na-meter-hints">
                  <span>0</span>
                  {q.min_words > 0 && <span style={{ color:"var(--amber)" }}>min {q.min_words}</span>}
                  {q.max_words > 0 && <span>max {q.max_words}</span>}
                </div>
              </div>
              ...
              {curAns.trim() && !skippedSet[q.id] && (
                <div className="na-answered-notice">
                  <IconCheck />
                  <span className="na-answered-notice-text">Answer recorded. You can edit it or proceed to the next question.</span>
                </div>
              )}
            </div>
          )}
          ...
          {/* Violation banner */}
          <div className={`na-viol-banner${showViolBanner ? " show" : ""}`}>
            <IconWarn />
          ...
          {questions.length > 0 && (
            <div style={{ display:"flex", justifyContent:"space-between", marginTop:16 }}>
              <button
                ...
                style={{
                  padding:"8px 18px", background:"var(--surface)", border:"1.5px solid var(--border)",
                  borderRadius:8, fontSize:12, fontWeight:600,
                  color: current === 0 ? "var(--dim)" : "var(--muted)",
                  cursor: current === 0 ? "not-allowed" : "pointer", fontFamily:"Inter,sans-serif",
                }}
              >← Previous</button>
              <button
                ...
                style={{
                  padding:"8px 18px", background:"var(--surface)", border:"1.5px solid var(--border)",
                  borderRadius:8, fontSize:12, fontWeight:600,
                  color: current === questions.length - 1 ? "var(--dim)" : "var(--muted)",
                  cursor: current === questions.length - 1 ? "not-allowed" : "pointer", fontFamily:"Inter,sans-serif",
                }}
              >Next →</button>
            </div>
          )}
        ...
        <div className="na-action-bar">
          {/* Skip */}
          <button
            className="na-btn na-btn-skip"
            onClick={() => {
              if (!q) return;
              setSkippedSet(prev => ({ ...prev, [q.id]: true }));
          ...
          {/* Save & Next — primary blue (like ExamPage's "Save & Continue") */}
          <button
            className="na-btn na-btn-primary"
            disabled={!curAns.trim()}
            style={{ opacity: !curAns.trim() ? 0.5 : 1 }}
            onClick={() => {
              autoSave();
              if (current < questions.length - 1) setCurrent(c => c + 1);
            }}
          ...
          {/* Submit — dark button (like ExamPage's "Next Question →") */}
          <button
            className="na-btn na-btn-next"
        ...
        {/* ══ SIDEBAR — IDENTICAL structure to ExamPage ════════════════════ */}
        <aside className="na-sidebar">
          ...
          {/* Stat cards — 2-column grid same as ExamPage */}
          ...
          {/* Question nav grid — IDENTICAL to ExamPage + skipped dot */}
          <div className="na-nav-section">
            <div className="na-section-label">Questions</div>
            <div className="na-nav-grid">
              {questions.map((_, i) => (
                <div key={i} className={qNavCls(i)} onClick={() => setCurrent(i)}>
                  {i + 1}
                </div>
              ))}
            </div>
            <div className="na-legend">
              {[
                ...
                { color:"var(--amber-s)",  border:"1px solid rgba(217,119,6,0.25)",   label:"Skipped"  },
          ...
          {/* Submit in sidebar */}
          <div className="na-sidebar-submit">
      ...
      {examDone && result && (
        <ResultOverlay
          ...
          onNavigate={(page) => { if (onNavigate) onNavigate(page); }}
        />
      )}
    </>
  );
