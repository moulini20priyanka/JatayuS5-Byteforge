...
function CertCard({ cert, onTakeExam }) {
  const isSched = cert.status === "scheduled";
  return (
    <div className="na-card" style={{ overflow: "hidden", borderLeft: `3px solid ${cert.orgColor}` }}>
      ...
      <div style={{ padding: "18px 22px", display: "flex", flexDirection: "column", gap: 14 }}>
        {isSched ? (
          ...
          <>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", background: "#f8fafc", borderRadius: 8, padding: "10px 14px" }}>
              <div>
        ...
        <button
          ...
          style={{
            width: "100%",
            padding: "10px",
            background: `linear-gradient(135deg, ${cert.orgColor}, ${cert.orgColor}cc)`,
            color: "#fff",
            border: "none",
            borderRadius: 10,
            fontSize: 13,
...
export default function StudentCertifications() {
  ...
  const handleTakeExam = (cert) => {
    // Pass cert info via navigation state
    navigate("/cert-verify-flow", { state: { cert } });
  };
  ...
  return (
    <StudentLayout activePath="/student-certifications">
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(2,1fr)", gap: 20 }}>
        {CERTS.map(cert => (
          <CertCard key={cert.id} cert={cert} onTakeExam={handleTakeExam} />
        ))}
      </div>
    </StudentLayout>
  );
