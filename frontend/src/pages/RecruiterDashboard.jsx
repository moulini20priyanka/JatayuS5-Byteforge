// RecruiterDashboard.jsx — Blue theme matching login page
...
// Blue theme palette — overrides RecruiterLayout's C constants for this page
const B = {
  accent:      "#2563eb",
  accentDark:  "#1d4ed8",
  accentLight: "#eff6ff",
...
const recStyles = `
  .rec-stat-card {
    ...
    animation: recFadeUp 0.35s ease both;
  }
  .rec-stat-card:hover {
  ...
  .rec-modal-overlay {
    ...
    z-index: 2000;
    animation: recFadeIn 0.15s ease;
  }
  .rec-modal {
    background: #fff;
    border-radius: 14px;
  ...
  const Md = ({ text }) => {
    ...
    return (
      <div style={{ fontSize: 13, lineHeight: 1.65, color: "#1e3a8a" }}>
        {lines.map((ln, i) => {
          if (ln.startsWith("### ")) return <p key={i} style={{ margin: "8px 0 3px", fontWeight: 700, fontSize: 13, color: "#1e3a8a" }}>{ln.slice(4)}</p>;
          if (ln.startsWith("## ")) return <p key={i} style={{ margin: "10px 0 4px", fontWeight: 700, fontSize: 14, color: "#1e3a8a" }}>{ln.slice(3)}</p>;
          if (ln.startsWith("- ") || ln.startsWith("• ")) { const parts = ln.slice(2).split(/(\*\*[^*]+\*\*)/g); return (<div key={i} style={{ display: "flex", gap: 6, margin: "3px 0" }}><span style={{ color: "#2563eb", flexShrink: 0 }}>•</span><span>{parts.map((p, j) => p.startsWith("**") ? <strong key={j}>{p.slice(2, -2)}</strong> : p)}</span></div>); }
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "14px 14px 0" }}>
        {messages.map((m, i) => (
          <div key={i} style={{ display: "flex", justifyContent: m.role === "user" ? "flex-end" : "flex-start", marginBottom: 12, gap: 8, alignItems: "flex-start" }}>
            {m.role === "assistant" && <div className="rec-ai-icon" style={{ width: 28, height: 28, borderRadius: 8, fontSize: 13, marginTop: 2, background: "linear-gradient(135deg,#1d4ed8,#2563eb)", flexShrink: 0 }}>AI</div>}
            <div style={{ maxWidth: m.role === "user" ? "75%" : "100%", flex: m.role === "assistant" ? 1 : "unset", background: m.role === "user" ? "linear-gradient(135deg,#2563eb,#1d4ed8)" : "#f0f9ff", color: m.role === "user" ? "#fff" : "#1e3a8a", padding: "10px 13px", borderRadius: m.role === "user" ? "14px 14px 3px 14px" : "14px 14px 14px 3px", border: m.role === "assistant" ? "1px solid #dbeafe" : "none" }}>
              ...
              {m.role === "user" ? <span style={{ fontSize: 13, lineHeight: 1.6 }}>{m.content.text}</span> : <><Md text={m.content.text} />{m.content.candidates?.length > 0 && <div style={{ marginTop: 10 }}>{m.content.candidates.map((c, ci) => <div key={ci} style={{ background: "#fff", border: "1px solid #dbeafe", borderRadius: 8, padding: "8px 12px", marginBottom: 6, transition: "background 0.15s" }}><div style={{ display: "flex", justifyContent: "space-between" }}><span style={{ fontWeight: 700, fontSize: 13, color: "#1e3a8a" }}>{c.name || c.student_id}</span><span style={{ fontWeight: 700, fontSize: 13, color: "#2563eb" }}>{Math.round(c.totalScore || c.total_score || 0)}/100</span></div></div>)}</div>}</>}
            </div>
          </div>
        ))}
        {loading && <div style={{ display: "flex", gap: 8, alignItems: "flex-start", marginBottom: 12 }}><div style={{ width: 28, height: 28, borderRadius: 8, background: "linear-gradient(135deg,#1d4ed8,#2563eb)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 700, color: "#fff" }}>AI</div><div style={{ padding: "10px 14px", background: "#f0f9ff", border: "1px solid #dbeafe", borderRadius: "14px 14px 14px 3px", display: "flex", alignItems: "center", gap: 5 }}><span style={{ fontSize: 11, color: "#60a5fa", marginRight: 2 }}>Processing</span>{[0,1,2].map(j => <span key={j} style={{ width: 5, height: 5, borderRadius: "50%", background: "#2563eb", display: "inline-block", animation: `recPulseDot 1s ${j*0.18}s infinite` }} />)}</div></div>}
        <div ref={bottomRef} />
      </div>

      {messages.length <= 1 && <div style={{ padding: "10px 14px 0", display: "flex", gap: 6, flexWrap: "wrap" }}>{suggestions.map(s => <button key={s} onClick={() => send(s)} style={{ fontSize: 11, padding: "5px 10px", borderRadius: 20, background: "#eff6ff", border: "1px solid #bfdbfe", color: "#1d4ed8", cursor: "pointer", transition: "all 0.15s", fontWeight: 500 }}
        onMouseEnter={e => { e.currentTarget.style.background = "#dbeafe"; }}
        onMouseLeave={e => { e.currentTarget.style.background = "#eff6ff"; }}
      >{s}</button>)}</div>}

      {pdfs.length > 0 && <div style={{ padding: "8px 14px 0", display: "flex", gap: 6, flexWrap: "wrap" }}>{pdfs.map((p, i) => <div key={i} style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 11, padding: "4px 10px", borderRadius: 20, background: "#eff6ff", border: "1px solid #bfdbfe", color: "#1d4ed8" }}>PDF: {p.name.length > 20 ? p.name.slice(0,18)+"…" : p.name}<button onClick={() => removePdf(i)} style={{ background: "none", border: "none", cursor: "pointer", color: "#2563eb", padding: 0, fontSize: 12 }}>×</button></div>)}</div>}

      {dragOver && <div style={{ margin: "8px 14px", padding: 16, border: "2px dashed #3b82f6", borderRadius: 10, textAlign: "center", background: "#eff6ff", fontSize: 12, color: "#1d4ed8", fontWeight: 600 }}>Drop PDF report here</div>}

      <div style={{ padding: "10px 14px 14px", borderTop: "1px solid #dbeafe", background: "#fff", flexShrink: 0 }}
        onDragOver={e => { e.preventDefault(); setDragOver(true); }}
        onDragLeave={() => setDragOver(false)}
        onDrop={e => { e.preventDefault(); setDragOver(false); handleFiles(e.dataTransfer.files); }}
      >
        <div style={{ display: "flex", gap: 8, alignItems: "flex-end" }}>
          <button onClick={() => fileRef.current?.click()} title="Attach PDF" style={{ width: 36, height: 36, borderRadius: 9, background: "#eff6ff", border: "1px solid #bfdbfe", cursor: "pointer", fontSize: 16, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, transition: "all 0.15s", color: "#1d4ed8" }}
            onMouseEnter={e => { e.currentTarget.style.background = "#dbeafe"; }}
            onMouseLeave={e => { e.currentTarget.style.background = "#eff6ff"; }}
          >
            <Icon d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13" size={16} color="#1d4ed8" strokeWidth={2} />
          </button>
          <input ref={fileRef} type="file" accept=".pdf" multiple style={{ display: "none" }} onChange={e => handleFiles(e.target.files)} />
          <textarea value={input} onChange={e => setInput(e.target.value)} onKeyDown={handleKey} placeholder="Ask anything… (Enter to send)" rows={1}
            style={{ flex: 1, padding: "9px 12px", borderRadius: 10, border: "1.5px solid #dbeafe", fontSize: 13, resize: "none", outline: "none", fontFamily: "inherit", lineHeight: 1.55, color: "#1e3a8a", background: "#fff", transition: "border-color 0.15s" }}
            onFocus={e => e.target.style.borderColor = "#3b82f6"}
            onBlur={e => e.target.style.borderColor = "#dbeafe"}
          />
          <button onClick={() => send()} disabled={loading || (!input.trim() && !pdfs.length)} title="Send message"
            style={{ width: 36, height: 36, borderRadius: 9, background: (input.trim() || pdfs.length) && !loading ? "linear-gradient(135deg,#2563eb,#1d4ed8)" : "#cbd5e1", border: "none", color: "#fff", cursor: (input.trim() || pdfs.length) && !loading ? "pointer" : "not-allowed", fontSize: 18, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, transition: "all 0.15s" }}
          >
            <Icon d="M5 12h14M12 5l7 7-7 7" size={18} color="#fff" strokeWidth={2.5} />
          </button>
        </div>
        <div style={{ marginTop: 6, fontSize: 10, color: "#93c5fd", textAlign: "center" }}>Powered by Gemini 1.5 Flash</div>
      </div>
    </div>
  );
}
...
export default function RecruiterDashboard() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      else if (err.code === 'ERR_NETWORK' || err.code === 'ECONNREFUSED') setError('Cannot connect to server at port 5000. Is the backend running?');
        ...
        <div className="rec-error-box">
          ...
          <div style={{ marginLeft: "auto", display: "flex", gap: 8 }}>
            {error.includes('log in') && (
              <button onClick={() => navigate('/login')} style={{ padding: "4px 10px", borderRadius: 6, border: `1px solid ${B.orange}`, background: "transparent", color: B.orange, cursor: "pointer", fontSize: 12, transition: "all 0.15s" }}>Login</button>
            )}
            <button onClick={fetchData} style={{ padding: "4px 10px", borderRadius: 6, border: `1px solid ${B.orange}`, background: "transparent", color: B.orange, cursor: "pointer", fontSize: 12, transition: "all 0.15s" }}>Retry</button>
          </div>
        </div>
      )}
      ...
      {activeTab === "dashboard" && (
        <>
          ...
          <div className="rec-ai-banner" onClick={openAI}>
            <span style={{ fontSize: 14, fontWeight: 800, color: "#fff", width: 40, height: 40, borderRadius: 10, background: "linear-gradient(135deg,#1d4ed8,#2563eb)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, border: "2px solid #bfdbfe" }}>AI</span>
            <div style={{ flex: 1 }}>
              <div style={{ fontWeight: 700, fontSize: 14, color: B.accent }}>AI Analyst — {allCandidates.length} live candidates loaded</div>
              <div style={{ fontSize: 12, color: B.dim, marginTop: 2 }}>Search candidates by skill, score, or role. Upload student PDF reports. Powered by Gemini 1.5 Flash.</div>
            </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, marginBottom: 28 }}>
            <div className="rec-panel">
              <div className="rec-panel-header">
                ...
                <button onClick={() => setActiveTab("candidates")} style={{ fontSize: 11, color: B.accent, background: "none", border: "none", cursor: "pointer", fontWeight: 600, transition: "color 0.15s" }}
                  onMouseEnter={e => e.currentTarget.style.color = B.accentDark}
                  onMouseLeave={e => e.currentTarget.style.color = B.accent}
                >View all</button>
              </div>
              {backendLoading ? <div style={{ padding: 32, textAlign: "center", color: B.dim, fontSize: 13 }}>Loading…</div> : (
                <div>
                  {recentFromAPI.map((r, i) => (
                    <div key={i} className="rec-row-hover" style={{ padding: "12px 20px", borderBottom: i < recentFromAPI.length - 1 ? `1px solid #eff6ff` : "none", display: "flex", justifyContent: "space-between", alignItems: "center", transition: "background 0.15s" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                        <div style={{ width: 30, height: 30, borderRadius: "50%", background: B.accentLight, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 700, color: B.accent, flexShrink: 0, border: `1px solid ${B.accentBorder}` }}>{(r.name || r.student_id || "?")[0].toUpperCase()}</div>
                        <div>
                  ...
                  {recentFromAPI.length === 0 && <div style={{ padding: 32, textAlign: "center", color: B.dim, fontSize: 13 }}>No candidates yet</div>}
                </div>
              )}
            </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              <div className="rec-panel" style={{ flex: 1 }}>
                ...
                <div style={{ padding: 16 }}>
                  {Object.keys(examTypeStats).length === 0 && !backendLoading && <div style={{ fontSize: 12, color: B.dim, textAlign: "center", padding: 16 }}>No data yet</div>}
      ...
      {activeTab === "candidates" && (
        <>
            ...
            <div className="rec-panel">
              <div style={{ padding: 20 }}>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  {[70,75,80].map(c => (
                    <button key={c} onClick={() => setSelectedCriteria(c)} className={`rec-criteria-btn${selectedCriteria === c ? " active" : ""}`}>{c}%</button>
                  ))}
                </div>
              </div>
            </div>
          ...
          <div style={{ marginBottom: 28 }}>
            <h2 style={{ margin: "0 0 14px", fontSize: 16, fontWeight: 700, color: B.green }}>Shortlisted Candidates (≥{selectedCriteria}%)</h2>
            {backendLoading ? <div style={{ padding: 32, textAlign: "center", color: B.dim }}>Loading…</div> : qualified.length > 0 ? (
              <div className="rec-panel" style={{ overflowX: "auto" }}>
                <table className="rec-table">
                  <thead><tr>{["Candidate","College","Exam Type","Score","Decision","Skills","Action"].map((h,i) => <th key={i}>{h}</th>)}</tr></thead>
                  <tbody>
                    {qualified.map(s => (
                      <tr key={s.id||s.student_id} className="rec-row-hover">
                        <td><div style={{ fontWeight: 600, color: B.text }}>{s.name||s.student_id}</div><div style={{ fontSize: 11, color: B.dim }}>{s.email||""}</div></td>
                        <td style={{ color: B.dim }}>{s.college||"—"}</td>
                        <td><span className="rec-tag">{s.examType||"—"}</span></td>
                        <td><span style={{ padding: "3px 10px", borderRadius: 6, fontSize: 12, fontWeight: 700, background: B.greenBg, color: B.green, border: `1px solid ${B.greenBorder}` }}>{Math.round(s.totalScore)}%</span></td>
                        <td>{s.decision?<span style={{ padding: "2px 8px", borderRadius: 20, fontSize: 10, fontWeight: 700, background: decisionBg(s.decision), color: decisionColor(s.decision) }}>{s.decision}</span>:"—"}</td>
                        <td><div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>{(s.skills||[]).slice(0,2).map(sk => <span key={sk} style={{ fontSize: 10, padding: "2px 6px", borderRadius: 4, background: B.accentLight, border: `1px solid ${B.accentBorder}`, color: B.accent }}>{sk}</span>)}</div></td>
                        <td><button className="rec-schedule-btn" onClick={() => { setSelectedStudent(s); setShowModal(true); }}>Schedule</button></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              </div>
            ) : <p style={{ padding: 24, textAlign: "center", color: B.dim, background: "#fff", borderRadius: 12, border: "1px solid #dbeafe" }}>No shortlisted candidates</p>}
          </div>

          <div>
            <h2 style={{ margin: "0 0 14px", fontSize: 16, fontWeight: 700, color: B.orange }}>Under Review (&lt;{selectedCriteria}%)</h2>
            {notQualified.length > 0 ? (
              <div className="rec-panel" style={{ overflowX: "auto" }}>
                <table className="rec-table">
                  <thead><tr>{["Candidate","College","Exam Type","Score","Decision","Action"].map((h,i) => <th key={i}>{h}</th>)}</tr></thead>
                  <tbody>
                    {notQualified.map(s => (
                      <tr key={s.id||s.student_id} className="rec-row-hover">
                        ...
                        <td style={{ color: B.dim }}>{s.college||"—"}</td>
                        <td><span className="rec-tag">{s.examType||"—"}</span></td>
                        <td><span style={{ padding: "3px 10px", borderRadius: 6, fontSize: 12, fontWeight: 700, background: B.redBg, color: B.red, border: `1px solid ${B.redBorder}` }}>{Math.round(s.totalScore)}%</span></td>
                        <td>{s.decision?<span style={{ padding: "2px 8px", borderRadius: 20, fontSize: 10, fontWeight: 700, background: decisionBg(s.decision), color: decisionColor(s.decision) }}>{s.decision}</span>:"—"}</td>
            ...
            ) : <p style={{ padding: 24, textAlign: "center", color: B.dim, background: "#fff", borderRadius: 12, border: "1px solid #dbeafe" }}>No review candidates</p>}
          </div>
        </>
      )}
      ...
      {activeTab === "reports" && (
        <>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 16, marginBottom: 28 }}>
            <StatCard label="Total Candidates" value={allCandidates.length} color={B.accent} delay={0} iconPath="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z" />
            <StatCard label="Avg Score" value={allCandidates.length>0?Math.round(allCandidates.reduce((a,c)=>a+c.totalScore,0)/allCandidates.length)+"%":"—"} color="#0369a1" delay={0.05} iconPath="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
            <StatCard label="Hire Rate" value={allCandidates.length>0?Math.round((allCandidates.filter(c=>c.decision==="Hire").length/allCandidates.length)*100)+"%":"—"} color={B.green} delay={0.1} iconPath="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
          </div>

          <h2 style={{ margin: "0 0 16px", fontSize: 16, fontWeight: 700, color: B.text }}>Performance by Exam Type</h2>
          {Object.keys(examTypeStats).length === 0 ? <p style={{ padding: 24, textAlign: "center", color: B.dim, background: "#fff", borderRadius: 12, border: "1px solid #dbeafe" }}>No data available</p> : (
              ...
              {Object.entries(examTypeStats).map(([type, stats]) => (
                <div key={type} className="rec-panel">
                  <div className="rec-panel-header">
                    ...
                    <span style={{ padding: "3px 10px", borderRadius: 12, fontSize: 11, fontWeight: 600, background: "linear-gradient(135deg,#2563eb,#1d4ed8)", color: "#fff" }}>{stats.total}</span>
                  </div>
                  <div style={{ padding: 18 }}>
                    {[["Total Candidates",stats.total,B.text],["Shortlisted (≥70%)",stats.qualified,B.green],["Average Score",`${stats.avg}%`,"#0369a1"]].map(([l,v,col],i) => (
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
            ...
            <select className="rec-type-select" style={{ width: "auto", padding: "6px 10px", fontSize: 12 }} value={selectedExamType} onChange={e => setSelectedExamType(e.target.value)}>
              {examTypeOptions.map(t => <option key={t} value={t}>{t==="ALL"?"All Exams":t}</option>)}
            </select>
          </div>

          {backendLoading ? <div style={{ padding: 32, textAlign: "center", color: B.dim }}>Loading…</div> : filteredCandidates.length > 0 ? (
            <div className="rec-panel" style={{ overflowX: "auto" }}>
              <table className="rec-table">
                <thead><tr>{["Candidate","College","Exam Type","Score","Coding","Problem Solving","Decision"].map((h,i) => <th key={i}>{h}</th>)}</tr></thead>
                <tbody>
                  {filteredCandidates.map(c => (
                    <tr key={c.id||c.student_id} className="rec-row-hover">
                      ...
                      <td><span style={{ padding: "3px 10px", borderRadius: 6, fontSize: 12, fontWeight: 700, background: c.totalScore>=70?B.greenBg:B.redBg, color: c.totalScore>=70?B.green:B.red, border: `1px solid ${c.totalScore>=70?B.greenBorder:B.redBorder}` }}>{Math.round(c.totalScore)}%</span></td>
                      ...
                      <td>{c.decision?<span style={{ padding: "2px 8px", borderRadius: 20, fontSize: 10, fontWeight: 700, background: decisionBg(c.decision), color: decisionColor(c.decision) }}>{c.decision}</span>:"—"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            </div>
          ) : <p style={{ padding: 24, textAlign: "center", color: B.dim, background: "#fff", borderRadius: 12, border: "1px solid #dbeafe" }}>No candidates found</p>}
        </>
      )}
          ...
          <div className="rec-modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="rec-modal-body">
              <div className="rec-modal-info">
                {[["Name",selectedStudent.name||selectedStudent.student_id],["Score",`${Math.round(selectedStudent.totalScore)}%`],["Exam Type",selectedStudent.examType||"—"],["Decision",selectedStudent.decision||"Pending"]].map(([l,v],i) => (
                  <div key={i} className="rec-modal-info-row"><span className="rec-modal-info-label">{l}:</span><span className="rec-modal-info-value">{v}</span></div>
                ))}
              </div>
              {[{label:"Interview Date",type:"date"},{label:"Interview Time",type:"time"}].map(f => (
                <div key={f.label} className="rec-modal-field">
                  <label className="rec-modal-label">{f.label}</label>
                  <input type={f.type} className="rec-modal-input" />
                </div>
              ))}
              <div className="rec-modal-field">
                <label className="rec-modal-label">Interview Type</label>
                <select className="rec-modal-select"><option>Technical Round</option><option>HR Round</option><option>Final Round</option></select>
              </div>
              <div className="rec-modal-field">
                <label className="rec-modal-label">Interviewer Email</label>
                <input type="email" placeholder="interviewer@company.com" className="rec-modal-input" />
              </div>
              <div className="rec-modal-actions">
                <button className="rec-modal-submit">Schedule Interview</button>
