...
// ── "completed" fully removed from STATUS_STYLE ──
const STATUS_STYLE = {
...
// ── Color palette — matches Dashboard exactly (#e8f0fe → #dbeafe gradient) ──
const P = {
  // ── DASHBOARD-MATCHING background ──
  pageBg:      "linear-gradient(135deg, #e8f0fe 0%, #dbeafe 100%)",
...
function fl() { return { fontSize: 10, color: P.muted, marginBottom: 4, fontWeight: 600 }; }
function ghostBtn() {
  return {
    position: "absolute", right: 8, top: 8, fontSize: 10,
    background: P.white, border: `1px solid ${P.border}`, borderRadius: 4,
    padding: "3px 7px", cursor: "pointer",
    display: "flex", alignItems: "center", gap: 4, color: P.muted,
  };
}

// ── Normalize status to lowercase to handle DB case mismatches ──
...
export default function ExamRequestsPage() {
  ...
  // ── Fetch requests ──────────────────────────────────────────
  // Uses full API URL with token auth; excludes "completed" rows entirely
  const fetchRequests = async () => {
    ...
    try {
      ...
      // Normalize status + strip any "completed" rows
  ...
  // ── Filter tabs: all / pending / approved / rejected (completed removed) ──
  ...
  const cardStyle = {
    ...
    boxShadow:    "0 2px 12px rgba(29,109,235,0.07)",
  };
  ...
  return (
    // ── Outer wrapper: full-page gradient matching Dashboard exactly ──
    <div style={{ background: P.pageBg, minHeight: "100vh" }}>
      <RecruiterLayout
        title="Create Placement Assessment"
        subtitle="Configure hiring assessment with detailed specifications"
      >
              ...
              {success ? (
                <div style={{ padding: "48px 20px", textAlign: "center" }}>
                  <div style={{
                    ...
                    background: P.greenBg, border: `2px solid ${P.greenBorder}`,
                    display: "flex", alignItems: "center", justifyContent: "center",
                    margin: "0 auto 14px",
                  }}>
                ...
                <>
                  ...
                  {step === 4 && (
                    <>
                      ...
                      {/* Summary strip */}
                      <div style={{ marginBottom: 14, padding: 12, background: P.accentLight, borderRadius: 8, border: `1px solid ${P.accentBorder}` }}>
          ...
          <div style={{ ...cardStyle, overflow: "hidden" }}>
            <div style={cardHeaderStyle}>
              ...
              <button onClick={fetchRequests}
                style={{ padding: "6px 12px", borderRadius: 7, border: `1px solid ${P.border}`, background: P.white, color: P.muted, fontSize: 11, fontWeight: 600, cursor: "pointer", display: "flex", alignItems: "center", gap: 5 }}>
                ...
                return (
                  <button key={f} onClick={() => setFilter(f)}
                    style={{
                      ...
                      border: `1px solid ${active ? (ss.dot || P.accent) : P.border}`,
                      background: active ? (ss.bg || P.accentLight) : "transparent",
                      color: active ? (ss.color || P.accent) : P.muted,
                      fontSize: 11, fontWeight: 600, transition: "all 0.15s",
                      display: "flex", alignItems: "center", gap: 5,
                    }}>
                    {f !== "all" && (
                      <span style={{ width: 6, height: 6, borderRadius: "50%", background: active ? ss.dot : P.border }} />
                    )}
                    {f === "all" ? "All" : f.charAt(0).toUpperCase() + f.slice(1)}
                    <span style={{
                      background: active ? (ss.dot || P.accent) : "#e2e8f0",
            ...
            {loading ? (
              <div style={{ padding: 48, textAlign: "center", color: P.dim, fontSize: 13 }}>
                Loading requests...
              </div>
            ) : filtered.length === 0 ? (
              <div style={{ padding: 48, textAlign: "center", color: P.dim }}>
                ...
                <div style={{ fontSize: 13, fontWeight: 500 }}>
                  {filter === "all"
                    ? "No requests yet. Submit your first placement request."
                    : `No ${filter} requests.`}
                </div>
              </div>
            ) : (
                ...
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                  ...
                  <tbody>
                    {filtered.map(r => {
                      ...
                      return (
                        <tr key={r.id}
                          style={{ borderBottom: `1px solid ${P.border}`, transition: "background 0.12s" }}
                          ...
                          <td style={{ padding: "12px 14px", color: P.muted, fontSize: 11, whiteSpace: "nowrap" }}>
                            {r.schedule_date ? new Date(r.schedule_date).toLocaleDateString() : ""}
                          </td>
                          ...
                          <td style={{ padding: "12px 14px" }}>
                            <span style={{
                              ...
                              background: r.sectional_cutoff_required ? "#eff6ff" : P.bg,
                              color: r.sectional_cutoff_required ? "#1d4ed8" : P.dim,
                              border: `1px solid ${r.sectional_cutoff_required ? "#bfdbfe" : P.border}`,
                            }}>
